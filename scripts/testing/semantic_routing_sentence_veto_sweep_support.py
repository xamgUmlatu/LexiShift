...
def build_sentence_veto_sweep_report(
    *,
    ...
    phrase_guard_pos_scopes: Sequence[str] = ("family_all",),
    ...
    normalized_phrase_guard_pos_scopes = [
        value
        for value in _normalize_string_list(phrase_guard_pos_scopes)
        if value in SENTENCE_VETO_PHRASE_GUARD_POS_SCOPES
    ]
    ...
    if (
        ...
        or not normalized_phrase_guard_pos_scopes
        or not normalized_active_rescue_modes
    ):
        raise ValueError(
            "Sentence-veto sweep requires non-empty scorer, context-view, evidence-view, phrase-control mode, phrase-guard POS scope, and active-rescue mode sets."
        )
        ...
        for context_view in normalized_context_views:
            for evidence_view in normalized_evidence_views:
                for phrase_control_mode in normalized_phrase_control_modes:
                    for phrase_guard_pos_scope in normalized_phrase_guard_pos_scopes:
                            ...
                            for min_active_score in normalized_min_active_scores:
                                for min_margin in normalized_min_margins:
                                    ...
                                    row = {
                                        ...
                                        "shadow_winner_accuracy": summary.get(
                                            "shadow_winner_accuracy"
                                        ),
                                        "predicted_replace_rate": summary.get(
    ...
    return {
        ...
        "grid": {
            ...
            "phrase_guard_pos_scopes": normalized_phrase_guard_pos_scopes,
