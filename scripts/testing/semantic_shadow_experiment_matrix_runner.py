...
def render_experiment_matrix_markdown(report: Mapping[str, object]) -> str:
    lines = [
        ...
        lines.append(
            ...
            + " | ".join(
                [
                    ...
                    render_rate(row.get("gold_candidate_precision")),
                    render_rate(row.get("gold_candidate_recall")),
                    render_rate(row.get("veto_overall_accuracy")),
                    render_rate(row.get("veto_abstain_recall")),
                    render_rate(row.get("veto_harmful_allow_rate")),
                ]
            )
            + " |"
        )
            ...
            lines.append(
                ...
                + " | ".join(
                    [
                        ...
                        render_rate(_extract_split_delta_metric(split_summary, "abstain_recall")),
                        render_rate(
    ...
    for row in rows:
        ...
        lines.extend(
            [
                ...
                f"- Seed trigger keep rate: `{render_rate(row.get('seed_trigger_keep_rate'))}` (`{row.get('seed_trigger_count_after_filter', 0)} / {row.get('seed_trigger_count_before_filter', 0)}`)",
                f"- Gold candidate precision / recall / F1: `{render_rate(row.get('gold_candidate_precision'))}` / `{render_rate(row.get('gold_candidate_recall'))}` / `{render_rate(row.get('gold_candidate_f1'))}`",
                f"- Gold trigger hit / top1 hit / exact-pool match: `{render_rate(row.get('gold_trigger_hit_rate'))}` / `{render_rate(row.get('gold_top1_hit_rate'))}` / `{render_rate(row.get('candidate_pool_exact_match_rate'))}`",
                f"- Veto accuracy / abstain recall / harmful allow / overblocking: `{render_rate(row.get('veto_overall_accuracy'))}` / `{render_rate(row.get('veto_abstain_recall'))}` / `{render_rate(row.get('veto_harmful_allow_rate'))}` / `{render_rate(row.get('veto_overblocking_rate'))}`",
        ...
        if split_summary:
            lines.extend(
                [
                    ...
                    f"`{render_rate(_extract_split_summary_metric(split_summary, 'tune', 'overall_accuracy'))}` / "
                    f"`{render_rate(_extract_split_summary_metric(split_summary, 'tune', 'abstain_recall'))}` / "
                    f"`{render_rate(_extract_split_summary_metric(split_summary, 'tune', 'harmful_allow_rate'))}` / "
                    f"`{render_rate(_extract_split_summary_metric(split_summary, 'tune', 'overblocking_rate'))}`",
                    "- Held-out split veto acc / abstain recall / harmful allow / overblocking: "
                    f"`{render_rate(_extract_split_summary_metric(split_summary, 'held_out', 'overall_accuracy'))}` / "
                    f"`{render_rate(_extract_split_summary_metric(split_summary, 'held_out', 'abstain_recall'))}` / "
                    f"`{render_rate(_extract_split_summary_metric(split_summary, 'held_out', 'harmful_allow_rate'))}` / "
                    f"`{render_rate(_extract_split_summary_metric(split_summary, 'held_out', 'overblocking_rate'))}`",
                    "- Held-out minus tune acc / abstain recall / harmful allow / overblocking: "
                    f"`{render_rate(_extract_split_delta_metric(split_summary, 'overall_accuracy'))}` / "
                    f"`{render_rate(_extract_split_delta_metric(split_summary, 'abstain_recall'))}` / "
                    f"`{render_rate(_extract_split_delta_metric(split_summary, 'harmful_allow_rate'))}` / "
                    f"`{render_rate(_extract_split_delta_metric(split_summary, 'overblocking_rate'))}`",
                ]
            )
...
__all__ = ("build_experiment_matrix_report", "render_experiment_matrix_markdown")
