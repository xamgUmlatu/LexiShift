#!/usr/bin/env python3
from __future__ import annotations

import argparse
from collections.abc import Mapping, Sequence
from dataclasses import dataclass
import json
from pathlib import Path
import sys

import numpy as np


PROJECT_ROOT = Path(__file__).resolve().parents[2]
SCRIPT_DIR = PROJECT_ROOT / "scripts" / "testing"
sys.path.insert(0, str(SCRIPT_DIR))

from artifact_provenance import build_artifact_provenance  # noqa: E402
from srs_learner_difficulty_normalization import TARGET_CURVE_ID  # noqa: E402
from srs_learner_difficulty_proficiency_ordering_en_ja import (  # noqa: E402
    DEFAULT_CALIBRATION_MATRIX,
    DEFAULT_CHALLENGE_OFFSET,
    DEFAULT_COMPONENT_MATRIX,
    DEFAULT_HOLDOUT_JSON,
    DEFAULT_PROFICIENCY_POINTS,
    DEFAULT_WINDOW_SIGMA,
    DEFAULT_WINDOW_TOP_K,
    LabelContext,
    _calibration_context,
    _component_context,
    _escape,
    _label_context_from_json,
    _load_json,
    _mapping,
    _metric_path,
    _observed_for_context,
    _optional_float,
    _parse_float_csv,
    _proficiency_dataset_report,
    _repo_or_home_path,
    _rounded,
    _target_curve_normalize,
    _utc_now,
)
from srs_learner_difficulty_proficiency_ordering_search_en_ja import (  # noqa: E402
    FormulaCandidate,
    _normalized_values_for_formula,
    _policy_contexts,
)


PAIR = "en-ja"
DEFAULT_SEARCH_JSON = (
    PROJECT_ROOT
    / "docs"
    / "test_outputs"
    / "srs_learner_difficulty_proficiency_ordering_search_en_ja_latest.json"
)
DEFAULT_JSON_OUT = (
    PROJECT_ROOT
    / "docs"
    / "test_outputs"
    / "srs_learner_difficulty_proficiency_ordering_stability_en_ja_latest.json"
)
DEFAULT_MARKDOWN_OUT = (
    PROJECT_ROOT
    / "docs"
    / "test_outputs"
    / "srs_learner_difficulty_proficiency_ordering_stability_en_ja_latest.md"
)
DEFAULT_FOLD_COUNT = 5
DEFAULT_RESULT_RETAIN_LIMIT = 120
DEFAULT_LEADERBOARD_LIMIT = 20


@dataclass(frozen=True)
class StabilityCandidate:
    row: Mapping[str, object]
    formula: FormulaCandidate
    lane_policy: str


def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Rank proficiency-ordering search finalists by calibration-internal "
            "fold stability. Holdout remains a non-selector diagnostic."
        )
    )
    parser.add_argument("--search-json", type=Path, default=DEFAULT_SEARCH_JSON)
    parser.add_argument("--calibration-matrix", type=Path, default=DEFAULT_CALIBRATION_MATRIX)
    parser.add_argument("--component-matrix", type=Path, default=DEFAULT_COMPONENT_MATRIX)
    parser.add_argument("--holdout-json", type=Path, default=DEFAULT_HOLDOUT_JSON)
    parser.add_argument(
        "--proficiency-points",
        default=",".join(str(value) for value in DEFAULT_PROFICIENCY_POINTS),
    )
    parser.add_argument("--challenge-offset", type=float, default=DEFAULT_CHALLENGE_OFFSET)
    parser.add_argument("--window-sigma", type=float, default=DEFAULT_WINDOW_SIGMA)
    parser.add_argument("--window-top-k", type=int, default=DEFAULT_WINDOW_TOP_K)
    parser.add_argument("--fold-count", type=int, default=DEFAULT_FOLD_COUNT)
    parser.add_argument(
        "--candidate-limit",
        type=int,
        default=0,
        help="Optional retained-candidate limit for smoke runs. 0 evaluates all.",
    )
    parser.add_argument("--result-retain-limit", type=int, default=DEFAULT_RESULT_RETAIN_LIMIT)
    parser.add_argument("--leaderboard-limit", type=int, default=DEFAULT_LEADERBOARD_LIMIT)
    parser.add_argument("--detail-limit", type=int, default=20)
    parser.add_argument("--json-out", type=Path, default=DEFAULT_JSON_OUT)
    parser.add_argument("--markdown-out", type=Path, default=DEFAULT_MARKDOWN_OUT)
    return parser.parse_args()


def main() -> int:
    args = _parse_args()
    report = build_report(
        search_json_path=_resolve_path(args.search_json),
        calibration_matrix_path=_resolve_path(args.calibration_matrix),
        component_matrix_path=_resolve_path(args.component_matrix),
        holdout_json_path=_resolve_path(args.holdout_json),
        proficiency_points=_parse_float_csv(args.proficiency_points),
        challenge_offset=float(args.challenge_offset),
        window_sigma=max(1e-6, float(args.window_sigma)),
        window_top_k=max(1, int(args.window_top_k)),
        fold_count=max(2, int(args.fold_count)),
        candidate_limit=max(0, int(args.candidate_limit)),
        result_retain_limit=max(1, int(args.result_retain_limit)),
        leaderboard_limit=max(1, int(args.leaderboard_limit)),
        detail_limit=max(1, int(args.detail_limit)),
    )
    json_out = _resolve_path(args.json_out)
    markdown_out = _resolve_path(args.markdown_out)
    json_out.parent.mkdir(parents=True, exist_ok=True)
    markdown_out.parent.mkdir(parents=True, exist_ok=True)
    json_out.write_text(
        json.dumps(report, ensure_ascii=False, indent=2, sort_keys=True) + "\n",
        encoding="utf-8",
    )
    markdown_out.write_text(render_markdown(report), encoding="utf-8")
    print(f"Wrote JSON artifact to {json_out}")
    print(f"Wrote Markdown artifact to {markdown_out}")
    return 0


def build_report(
    *,
    search_json_path: Path,
    calibration_matrix_path: Path,
    component_matrix_path: Path,
    holdout_json_path: Path,
    proficiency_points: Sequence[float] = DEFAULT_PROFICIENCY_POINTS,
    challenge_offset: float = DEFAULT_CHALLENGE_OFFSET,
    window_sigma: float = DEFAULT_WINDOW_SIGMA,
    window_top_k: int = DEFAULT_WINDOW_TOP_K,
    fold_count: int = DEFAULT_FOLD_COUNT,
    candidate_limit: int = 0,
    result_retain_limit: int = DEFAULT_RESULT_RETAIN_LIMIT,
    leaderboard_limit: int = DEFAULT_LEADERBOARD_LIMIT,
    detail_limit: int = 20,
) -> dict[str, object]:
    search_report = _load_json(search_json_path)
    calibration = np.load(calibration_matrix_path)
    component = np.load(component_matrix_path)
    component_context = _component_context(component)
    calibration_context = _calibration_context(calibration, component_context)
    holdout_context = _label_context_from_json(
        _load_json(holdout_json_path),
        component_context=component_context,
        context_id="holdout",
    )
    candidates = _stability_candidates(search_report)
    if candidate_limit > 0:
        candidates = candidates[:candidate_limit]
    policy_contexts = _policy_contexts(
        component_context=component_context,
        calibration_context=calibration_context,
        holdout_context=holdout_context,
    )
    fold_masks = _stratified_fold_masks(calibration_context, fold_count=fold_count)
    train_masks = tuple(~np.asarray(mask, dtype=bool) for mask in fold_masks)
    fold_contexts = _fold_contexts_by_policy(
        policy_contexts,
        fold_masks,
        context_id_prefix="validation",
    )
    train_contexts = _fold_contexts_by_policy(
        policy_contexts,
        train_masks,
        context_id_prefix="train",
    )
    reference = _reference_stability_reports(
        search_report=search_report,
        component_context=component_context,
        fold_contexts=fold_contexts["current"],
        proficiency_points=proficiency_points,
        challenge_offset=challenge_offset,
        window_sigma=window_sigma,
        window_top_k=window_top_k,
        detail_limit=detail_limit,
    )
    results = []
    for candidate in candidates:
        validation_contexts = fold_contexts.get(candidate.lane_policy)
        candidate_train_contexts = train_contexts.get(candidate.lane_policy)
        if not validation_contexts or not candidate_train_contexts:
            continue
        normalized = _normalized_values_for_formula(candidate.formula, component_context)
        fold_reports = []
        train_reports = []
        for fold_context in validation_contexts:
            observed = _observed_for_context(normalized, fold_context)
            fold_reports.append(
                _proficiency_dataset_report(
                    fold_context,
                    observed,
                    proficiency_points=proficiency_points,
                    challenge_offset=challenge_offset,
                    window_sigma=window_sigma,
                    window_top_k=window_top_k,
                    detail_limit=detail_limit,
                )
            )
        for train_context in candidate_train_contexts:
            observed = _observed_for_context(normalized, train_context)
            train_reports.append(
                _proficiency_dataset_report(
                    train_context,
                    observed,
                    proficiency_points=proficiency_points,
                    challenge_offset=challenge_offset,
                    window_sigma=window_sigma,
                    window_top_k=window_top_k,
                    detail_limit=detail_limit,
                )
            )
        results.append(
            _candidate_stability_report(
                candidate,
                fold_reports=fold_reports,
                train_reports=train_reports,
                reference_summary=reference["current_production"]["fold_stability"],
            )
        )
    retained = _retained_results(results, limit=result_retain_limit)
    selector_events = _fold_training_selector_events(results)
    selector_frequency = _fold_training_selection_frequency(selector_events)
    return {
        "schema_version": 1,
        "generated_at": _utc_now(),
        "language_pair": PAIR,
        "runtime_behavior_changed": False,
        "stability_selector_run": True,
        "holdout_used_for_selection": False,
        "method": {
            "purpose": (
                "Calibration-internal stability selector for proficiency-ordering search finalists."
            ),
            "selection_rule": (
                "Rank candidates by calibration fold stability, and run a "
                "fold-training selector that picks each fold winner using only "
                "the other folds. Holdout is reported only after ranking."
            ),
            "candidate_scope": (
                "This selector consumes retained finalists from the search artifact; "
                "it does not regenerate the full formula grid."
            ),
            "target_curve_id": TARGET_CURVE_ID,
        },
        "parameters": {
            "fold_count": int(fold_count),
            "candidate_limit": int(candidate_limit),
            "result_retain_limit": int(result_retain_limit),
            "leaderboard_limit": int(leaderboard_limit),
            "proficiency_points": [round(float(value), 6) for value in proficiency_points],
            "challenge_offset": round(float(challenge_offset), 6),
            "window_sigma": round(float(window_sigma), 6),
            "window_top_k": int(window_top_k),
        },
        "inputs": {
            "search_json": _repo_or_home_path(search_json_path),
            "calibration_matrix": _repo_or_home_path(calibration_matrix_path),
            "component_matrix": _repo_or_home_path(component_matrix_path),
            "holdout_json": _repo_or_home_path(holdout_json_path),
            "search_candidate_results": len(_mapping_rows(search_report.get("candidate_results"))),
            "evaluated_candidate_count": len(candidates),
            "calibration_label_count": len(calibration_context.labels),
            "holdout_label_count": len(holdout_context.labels),
            "normalization_population_count": len(component_context.lemmas),
        },
        "provenance": build_artifact_provenance(
            producer_script=Path(__file__),
            input_paths={
                "search_json": search_json_path,
                "calibration_matrix": calibration_matrix_path,
                "component_matrix": component_matrix_path,
                "holdout_json": holdout_json_path,
            },
            code_paths={
                "artifact_provenance": SCRIPT_DIR / "artifact_provenance.py",
                "normalization": SCRIPT_DIR / "srs_learner_difficulty_normalization.py",
                "proficiency_ordering": SCRIPT_DIR
                / "srs_learner_difficulty_proficiency_ordering_en_ja.py",
                "proficiency_ordering_search": SCRIPT_DIR
                / "srs_learner_difficulty_proficiency_ordering_search_en_ja.py",
            },
            version_constants={"target_curve": TARGET_CURVE_ID},
            argv=sys.argv,
        ),
        "folds": _fold_summary_rows(calibration_context, fold_masks),
        "reference_candidates": reference,
        "fold_training_selector": {
            "events": selector_events,
            "selection_frequency": selector_frequency,
            "mean_validation_score": _rounded(_mean_metric(selector_events, "validation_score")),
            "mean_train_score": _rounded(_mean_metric(selector_events, "train_score")),
            "mean_holdout_score": _rounded(_mean_metric(selector_events, "holdout_score")),
        },
        "leaderboards": {
            "stability_selector": _leaderboard(
                results,
                sort_key="stability_selector_score",
                dataset="stability",
                limit=leaderboard_limit,
            ),
            "calibration_full_score": _leaderboard(
                results,
                sort_key="full_calibration_score",
                dataset="calibration",
                limit=leaderboard_limit,
            ),
            "holdout_for_stability_winners": _leaderboard(
                sorted(
                    results,
                    key=lambda row: _optional_float(row.get("stability_selector_score")) or -999.0,
                    reverse=True,
                ),
                sort_key="stability_selector_score",
                dataset="holdout",
                limit=leaderboard_limit,
                keep_order=True,
            ),
            "fold_training_selection_frequency": [
                _frequency_summary(row) for row in selector_frequency[:leaderboard_limit]
            ],
            "holdout_oracle_diagnostic": _leaderboard(
                results,
                sort_key="holdout_score",
                dataset="holdout",
                limit=leaderboard_limit,
            ),
        },
        "primary_candidates": {
            "stability_selector": _candidate_summary(
                _top_candidate(results, "stability_selector_score"),
                dataset="stability",
            ),
            "full_calibration_selector": _candidate_summary(
                _top_candidate(results, "full_calibration_score"),
                dataset="calibration",
            ),
            "holdout_oracle": _candidate_summary(
                _top_candidate(results, "holdout_score"),
                dataset="holdout",
            ),
            "fold_training_selector": _frequency_summary(
                selector_frequency[0] if selector_frequency else {}
            ),
        },
        "candidate_results": retained,
    }


def _stability_candidates(search_report: Mapping[str, object]) -> list[StabilityCandidate]:
    candidates = []
    for row in _mapping_rows(search_report.get("candidate_results")):
        weights = _float_mapping(row.get("weights"))
        if not weights:
            continue
        formula = FormulaCandidate(
            formula_id=str(row.get("formula_id") or row.get("candidate_id") or ""),
            feature_set_id=str(row.get("feature_set_id") or ""),
            weights=weights,
            max_shift_from_frequency=_optional_float(row.get("max_shift_from_frequency")),
            transforms=_mapping(row.get("transforms")),
            missing_features=tuple(str(value) for value in row.get("missing_features") or ()),
        )
        candidates.append(
            StabilityCandidate(
                row=row,
                formula=formula,
                lane_policy=str(row.get("lane_policy") or "current"),
            )
        )
    return candidates


def _stratified_fold_masks(
    context: LabelContext,
    *,
    fold_count: int,
) -> tuple[object, ...]:
    groups: dict[tuple[str, str, str], list[int]] = {}
    expected = np.asarray(context.expected_values, dtype=np.float32)
    states = np.asarray(context.expected_candidate_states, dtype=str)
    for index, label in enumerate(context.labels):
        key = (
            str(states[index]).strip(),
            str(context.expected_bands[index]).strip(),
            "numeric" if np.isfinite(expected[index]) else "non_numeric",
        )
        groups.setdefault(key, []).append(index)
    fold_indices: list[list[int]] = [[] for _ in range(fold_count)]
    group_offset = 0
    for key in sorted(groups):
        indices = sorted(groups[key], key=lambda value: context.labels[value])
        for offset, index in enumerate(indices):
            fold_indices[(group_offset + offset) % fold_count].append(index)
        group_offset = (group_offset + len(indices)) % fold_count
    masks = []
    for indices in fold_indices:
        mask = np.zeros(len(context.labels), dtype=bool)
        mask[sorted(indices)] = True
        masks.append(mask)
    return tuple(masks)


def _fold_contexts_by_policy(
    policy_contexts: Mapping[str, Mapping[str, object]],
    fold_masks: Sequence[object],
    *,
    context_id_prefix: str,
) -> dict[str, tuple[LabelContext, ...]]:
    contexts = {}
    for policy_id, policy_row in policy_contexts.items():
        context = policy_row.get("calibration_context")
        if not isinstance(context, LabelContext):
            continue
        contexts[policy_id] = tuple(
            _subset_context(
                context,
                mask,
                context_id=f"calibration_{policy_id}_{context_id_prefix}_{index + 1}",
            )
            for index, mask in enumerate(fold_masks)
        )
    return contexts


def _subset_context(
    context: LabelContext,
    mask: object,
    *,
    context_id: str,
) -> LabelContext:
    parsed = np.asarray(mask, dtype=bool)
    indices = np.where(parsed)[0]
    component_indices = np.asarray(context.component_indices, dtype=np.int64)[indices]
    return LabelContext(
        context_id=context_id,
        labels=tuple(context.labels[index] for index in indices),
        lemmas=tuple(context.lemmas[index] for index in indices),
        readings=tuple(context.readings[index] for index in indices),
        component_indices=component_indices,
        expected_values=np.asarray(context.expected_values, dtype=np.float32)[indices],
        expected_bands=tuple(context.expected_bands[index] for index in indices),
        expected_candidate_states=np.asarray(
            context.expected_candidate_states,
            dtype="<U64",
        )[indices],
        observed_candidate_states=np.asarray(
            context.observed_candidate_states,
            dtype="<U64",
        )[indices],
        missing_rows=tuple(
            {"label": context.labels[index], "row_index": int(offset)}
            for offset, index in enumerate(indices)
            if int(context.component_indices[index]) < 0
        ),
    )


def _candidate_stability_report(
    candidate: StabilityCandidate,
    *,
    fold_reports: Sequence[Mapping[str, object]],
    train_reports: Sequence[Mapping[str, object]],
    reference_summary: Mapping[str, object],
) -> dict[str, object]:
    fold_summary = _fold_stability_summary(fold_reports)
    train_rows = [_compact_fold_report(index, report) for index, report in enumerate(train_reports)]
    full_calibration = _mapping(candidate.row.get("calibration"))
    holdout = _mapping(candidate.row.get("holdout"))
    full_score = _metric_path(full_calibration, "proficiency_ordering_score")
    mean_score = _optional_float(fold_summary.get("mean_score")) or -999.0
    min_score = _optional_float(fold_summary.get("min_score")) or -999.0
    std_score = _optional_float(fold_summary.get("score_std")) or 999.0
    optimism = max(0.0, full_score - mean_score)
    stability_score = (0.70 * mean_score) + (0.25 * min_score) - (0.35 * std_score)
    stability_score -= 0.20 * optimism
    reference_mean = _optional_float(reference_summary.get("mean_score")) or -999.0
    return {
        "candidate_id": candidate.row.get("candidate_id"),
        "source": "proficiency_ordering_stability",
        "feature_set_id": candidate.row.get("feature_set_id"),
        "formula_id": candidate.row.get("formula_id"),
        "lane_policy": candidate.lane_policy,
        "weights": candidate.row.get("weights"),
        "max_shift_from_frequency": candidate.row.get("max_shift_from_frequency"),
        "transforms": candidate.row.get("transforms"),
        "missing_features": candidate.row.get("missing_features") or [],
        "guardrails": candidate.row.get("guardrails") or {},
        "full_calibration_score": _rounded(full_score),
        "fold_stability": fold_summary,
        "train_folds": train_rows,
        "stability_selector_score": _rounded(stability_score),
        "reference_fold_mean_delta": _rounded(mean_score - reference_mean),
        "full_calibration_optimism": _rounded(optimism),
        "calibration": full_calibration,
        "holdout": holdout,
        "holdout_score": _metric_path(holdout, "proficiency_ordering_score"),
        "holdout_delta_from_fold_mean": _rounded(
            _metric_path(holdout, "proficiency_ordering_score") - mean_score
        ),
    }


def _fold_stability_summary(
    fold_reports: Sequence[Mapping[str, object]],
) -> dict[str, object]:
    rows = [_compact_fold_report(index, report) for index, report in enumerate(fold_reports)]
    score_values = np.asarray(
        [_optional_float(row.get("score")) for row in rows],
        dtype=np.float32,
    )
    score_values = score_values[np.isfinite(score_values)]
    mean_score = float(score_values.mean()) if len(score_values) else None
    min_score = float(score_values.min()) if len(score_values) else None
    max_score = float(score_values.max()) if len(score_values) else None
    std_score = float(score_values.std(ddof=0)) if len(score_values) else None
    return {
        "fold_count": len(rows),
        "mean_score": _rounded(mean_score),
        "min_score": _rounded(min_score),
        "max_score": _rounded(max_score),
        "score_std": _rounded(std_score),
        "mean_normal_vocab_mae": _rounded(_mean_metric(rows, "normal_vocab_mae")),
        "mean_pairwise": _rounded(_mean_metric(rows, "normal_vocab_pairwise")),
        "mean_window_quality": _rounded(_mean_metric(rows, "window_quality")),
        "folds": rows,
    }


def _compact_fold_report(
    index: int,
    report: Mapping[str, object],
) -> dict[str, object]:
    normal_vocab = _mapping(report.get("normal_vocab"))
    metrics = _mapping(normal_vocab.get("metrics"))
    scores = _mapping(normal_vocab.get("scores"))
    windows = _mapping(report.get("frontier_windows"))
    lane = _mapping(report.get("lane"))
    return {
        "fold": index + 1,
        "score": report.get("proficiency_ordering_score"),
        "normal_vocab_mae": metrics.get("mae"),
        "normal_vocab_pairwise": scores.get("pairwise_order_score"),
        "window_quality": windows.get("average_window_score"),
        "normal_vocab_f1": lane.get("normal_vocab_f1"),
    }


def _reference_stability_reports(
    *,
    search_report: Mapping[str, object],
    component_context: object,
    fold_contexts: Sequence[LabelContext],
    proficiency_points: Sequence[float],
    challenge_offset: float,
    window_sigma: float,
    window_top_k: int,
    detail_limit: int,
) -> dict[str, dict[str, object]]:
    references = _mapping(search_report.get("reference_candidates"))
    rows = {}
    for reference_id, raw_values in (
        ("current_production", component_context.current_values),
        ("frequency_only", np.nan_to_num(component_context.frequency_values, nan=0.0)),
    ):
        normalized = _target_curve_normalize(
            raw_values,
            target_positions=component_context.target_curve_positions,
        )
        fold_reports = []
        for fold_context in fold_contexts:
            observed = _observed_for_context(normalized, fold_context)
            fold_reports.append(
                _proficiency_dataset_report(
                    fold_context,
                    observed,
                    proficiency_points=proficiency_points,
                    challenge_offset=challenge_offset,
                    window_sigma=window_sigma,
                    window_top_k=window_top_k,
                    detail_limit=detail_limit,
                )
            )
        rows[reference_id] = {
            **dict(_mapping(references.get(reference_id))),
            "fold_stability": _fold_stability_summary(fold_reports),
        }
    return rows


def _fold_summary_rows(
    context: LabelContext,
    fold_masks: Sequence[object],
) -> list[dict[str, object]]:
    expected = np.asarray(context.expected_values, dtype=np.float32)
    states = np.asarray(context.expected_candidate_states, dtype=str)
    rows = []
    for index, mask in enumerate(fold_masks):
        parsed = np.asarray(mask, dtype=bool)
        normal = parsed & (states == "normal_vocab") & np.isfinite(expected)
        rows.append(
            {
                "fold": index + 1,
                "label_count": int(parsed.sum()),
                "numeric_count": int((parsed & np.isfinite(expected)).sum()),
                "normal_vocab_numeric_count": int(normal.sum()),
            }
        )
    return rows


def _retained_results(
    results: Sequence[Mapping[str, object]],
    *,
    limit: int,
) -> list[dict[str, object]]:
    selected = []
    seen = set()
    for key in ("stability_selector_score", "holdout_score", "full_calibration_score"):
        for row in sorted(
            results,
            key=lambda value: _optional_float(value.get(key)) or -999.0,
            reverse=True,
        ):
            candidate_id = str(row.get("candidate_id") or "")
            if candidate_id and candidate_id not in seen:
                selected.append(dict(row))
                seen.add(candidate_id)
            if len(selected) >= limit:
                return selected
    return selected


def _fold_training_selector_events(
    results: Sequence[Mapping[str, object]],
) -> list[dict[str, object]]:
    fold_count = 0
    for row in results:
        fold_count = max(fold_count, len(_mapping_rows(row.get("train_folds"))))
    events = []
    for fold_index in range(fold_count):
        ranked = sorted(
            results,
            key=lambda row: _fold_score(row, "train_folds", fold_index),
            reverse=True,
        )
        if not ranked:
            continue
        selected = ranked[0]
        validation_folds = _mapping_rows(_mapping(selected.get("fold_stability")).get("folds"))
        validation = validation_folds[fold_index] if fold_index < len(validation_folds) else {}
        train_folds = _mapping_rows(selected.get("train_folds"))
        train = train_folds[fold_index] if fold_index < len(train_folds) else {}
        events.append(
            {
                "fold": fold_index + 1,
                "candidate_id": selected.get("candidate_id"),
                "feature_set_id": selected.get("feature_set_id"),
                "lane_policy": selected.get("lane_policy"),
                "train_score": train.get("score"),
                "validation_score": validation.get("score"),
                "validation_pairwise": validation.get("normal_vocab_pairwise"),
                "validation_window_quality": validation.get("window_quality"),
                "holdout_score": selected.get("holdout_score"),
                "full_calibration_score": selected.get("full_calibration_score"),
                "stability_selector_score": selected.get("stability_selector_score"),
            }
        )
    return events


def _fold_training_selection_frequency(
    events: Sequence[Mapping[str, object]],
) -> list[dict[str, object]]:
    grouped: dict[str, dict[str, object]] = {}
    for event in events:
        candidate_id = str(event.get("candidate_id") or "")
        if not candidate_id:
            continue
        row = grouped.setdefault(
            candidate_id,
            {
                "candidate_id": candidate_id,
                "feature_set_id": event.get("feature_set_id"),
                "lane_policy": event.get("lane_policy"),
                "folds_selected": [],
                "validation_scores": [],
                "train_scores": [],
                "holdout_scores": [],
            },
        )
        row["folds_selected"].append(event.get("fold"))  # type: ignore[index,union-attr]
        row["validation_scores"].append(  # type: ignore[index,union-attr]
            event.get("validation_score")
        )
        row["train_scores"].append(event.get("train_score"))  # type: ignore[index,union-attr]
        row["holdout_scores"].append(event.get("holdout_score"))  # type: ignore[index,union-attr]
    summaries = []
    for row in grouped.values():
        validation_scores = _mapping_list_numbers(row.get("validation_scores"))
        train_scores = _mapping_list_numbers(row.get("train_scores"))
        holdout_scores = _mapping_list_numbers(row.get("holdout_scores"))
        summaries.append(
            {
                "candidate_id": row.get("candidate_id"),
                "feature_set_id": row.get("feature_set_id"),
                "lane_policy": row.get("lane_policy"),
                "selected_fold_count": len(row.get("folds_selected") or []),
                "folds_selected": row.get("folds_selected") or [],
                "mean_validation_score": _rounded(_mean_number_list(validation_scores)),
                "min_validation_score": _rounded(
                    min(validation_scores) if validation_scores else None
                ),
                "mean_train_score": _rounded(_mean_number_list(train_scores)),
                "mean_holdout_score": _rounded(_mean_number_list(holdout_scores)),
            }
        )
    return sorted(
        summaries,
        key=lambda row: (
            int(row.get("selected_fold_count") or 0),
            _optional_float(row.get("mean_validation_score")) or -999.0,
        ),
        reverse=True,
    )


def _fold_score(
    row: Mapping[str, object],
    key: str,
    fold_index: int,
) -> float:
    fold_rows = _mapping_rows(row.get(key))
    if fold_index >= len(fold_rows):
        return -999.0
    return _optional_float(fold_rows[fold_index].get("score")) or -999.0


def _frequency_summary(row: Mapping[str, object]) -> dict[str, object]:
    return {
        "candidate_id": row.get("candidate_id"),
        "feature_set_id": row.get("feature_set_id"),
        "lane_policy": row.get("lane_policy"),
        "selected_fold_count": row.get("selected_fold_count"),
        "folds_selected": row.get("folds_selected") or [],
        "mean_validation_score": row.get("mean_validation_score"),
        "min_validation_score": row.get("min_validation_score"),
        "mean_train_score": row.get("mean_train_score"),
        "mean_holdout_score": row.get("mean_holdout_score"),
    }


def _leaderboard(
    candidates: Sequence[Mapping[str, object]],
    *,
    sort_key: str,
    dataset: str,
    limit: int,
    keep_order: bool = False,
) -> list[dict[str, object]]:
    rows = list(candidates)
    if not keep_order:
        rows = sorted(
            rows,
            key=lambda row: _optional_float(row.get(sort_key)) or -999.0,
            reverse=True,
        )
    return [_candidate_summary(row, dataset=dataset) for row in rows[:limit]]


def _top_candidate(
    candidates: Sequence[Mapping[str, object]],
    sort_key: str,
) -> Mapping[str, object]:
    if not candidates:
        return {}
    return max(candidates, key=lambda row: _optional_float(row.get(sort_key)) or -999.0)


def _candidate_summary(
    row: Mapping[str, object],
    *,
    dataset: str,
) -> dict[str, object]:
    stability = _mapping(row.get("fold_stability"))
    calibration = _mapping(row.get("calibration"))
    holdout = _mapping(row.get("holdout"))
    dataset_report = calibration if dataset == "calibration" else holdout
    if dataset == "stability":
        dataset_score = row.get("stability_selector_score")
    else:
        dataset_score = dataset_report.get("proficiency_ordering_score")
    normal_vocab = _mapping(dataset_report.get("normal_vocab"))
    metrics = _mapping(normal_vocab.get("metrics"))
    scores = _mapping(normal_vocab.get("scores"))
    windows = _mapping(dataset_report.get("frontier_windows"))
    return {
        "candidate_id": row.get("candidate_id"),
        "feature_set_id": row.get("feature_set_id"),
        "lane_policy": row.get("lane_policy"),
        "dataset_score": dataset_score,
        "stability_selector_score": row.get("stability_selector_score"),
        "fold_mean": stability.get("mean_score"),
        "fold_min": stability.get("min_score"),
        "fold_std": stability.get("score_std"),
        "full_calibration_score": row.get("full_calibration_score"),
        "full_calibration_optimism": row.get("full_calibration_optimism"),
        "holdout_score": row.get("holdout_score"),
        "holdout_delta_from_fold_mean": row.get("holdout_delta_from_fold_mean"),
        "normal_vocab_mae": metrics.get("mae"),
        "normal_vocab_pairwise": scores.get("pairwise_order_score"),
        "window_quality": windows.get("average_window_score"),
    }


def render_markdown(report: Mapping[str, object]) -> str:
    inputs = _mapping(report.get("inputs"))
    method = _mapping(report.get("method"))
    lines = [
        "# en-ja Learner Difficulty Proficiency Ordering Stability Sidecar",
        "",
        f"- Generated: `{_escape(report.get('generated_at'))}`",
        f"- Runtime behavior changed: `{_escape(report.get('runtime_behavior_changed'))}`",
        f"- Holdout used for selection: `{_escape(report.get('holdout_used_for_selection'))}`",
        f"- Search candidates available: `{_escape(inputs.get('search_candidate_results'))}`",
        f"- Evaluated candidates: `{_escape(inputs.get('evaluated_candidate_count'))}`",
        f"- Calibration labels: `{_escape(inputs.get('calibration_label_count'))}`",
        f"- Holdout labels: `{_escape(inputs.get('holdout_label_count'))}`",
        "",
        "## Method",
        "",
        str(method.get("purpose") or ""),
        "",
        str(method.get("selection_rule") or ""),
        "",
        str(method.get("candidate_scope") or ""),
        "",
    ]
    lines.extend(_folds_section(report))
    lines.extend(_reference_section(report))
    lines.extend(_fold_training_selector_section(report))
    lines.extend(
        _leaderboard_section(
            report,
            "stability_selector",
            "Stability Selector",
            dataset_label="calibration folds",
        )
    )
    lines.extend(
        _leaderboard_section(
            report,
            "holdout_for_stability_winners",
            "Holdout Report For Stability Winners",
            dataset_label="holdout",
        )
    )
    lines.extend(
        _leaderboard_section(
            report,
            "calibration_full_score",
            "Full Calibration Selector Diagnostic",
            dataset_label="calibration",
        )
    )
    lines.extend(
        _leaderboard_section(
            report,
            "holdout_oracle_diagnostic",
            "Holdout Oracle Diagnostic",
            dataset_label="holdout",
        )
    )
    return "\n".join(lines).rstrip() + "\n"


def _folds_section(report: Mapping[str, object]) -> list[str]:
    rows = _mapping_rows(report.get("folds"))
    lines = ["## Fold Balance", ""]
    lines.append("| Fold | Labels | Numeric | Normal vocab numeric |")
    lines.append("| ---: | ---: | ---: | ---: |")
    for row in rows:
        lines.append(
            f"| `{_escape(row.get('fold'))}` | `{_escape(row.get('label_count'))}` | "
            f"`{_escape(row.get('numeric_count'))}` | "
            f"`{_escape(row.get('normal_vocab_numeric_count'))}` |"
        )
    lines.append("")
    return lines


def _fold_training_selector_section(report: Mapping[str, object]) -> list[str]:
    selector = _mapping(report.get("fold_training_selector"))
    frequency_rows = _mapping_rows(selector.get("selection_frequency"))
    event_rows = _mapping_rows(selector.get("events"))
    lines = [
        "## Fold-Training Selector",
        "",
        f"- Mean train score: `{_escape(selector.get('mean_train_score'))}`",
        f"- Mean validation score: `{_escape(selector.get('mean_validation_score'))}`",
        f"- Mean holdout score: `{_escape(selector.get('mean_holdout_score'))}`",
        "",
        "| Candidate | Selected folds | Validation mean | Validation min | Holdout mean |",
        "| --- | ---: | ---: | ---: | ---: |",
    ]
    for row in frequency_rows:
        folds = ",".join(str(value) for value in row.get("folds_selected") or [])
        lines.append(
            f"| `{_escape(row.get('candidate_id'))}` | `{_escape(folds)}` | "
            f"`{_escape(row.get('mean_validation_score'))}` | "
            f"`{_escape(row.get('min_validation_score'))}` | "
            f"`{_escape(row.get('mean_holdout_score'))}` |"
        )
    lines.extend(["", "| Fold | Selected candidate | Train | Validation | Holdout |"])
    lines.append("| ---: | --- | ---: | ---: | ---: |")
    for row in event_rows:
        lines.append(
            f"| `{_escape(row.get('fold'))}` | `{_escape(row.get('candidate_id'))}` | "
            f"`{_escape(row.get('train_score'))}` | "
            f"`{_escape(row.get('validation_score'))}` | "
            f"`{_escape(row.get('holdout_score'))}` |"
        )
    lines.append("")
    return lines


def _reference_section(report: Mapping[str, object]) -> list[str]:
    references = _mapping(report.get("reference_candidates"))
    lines = ["## References", ""]
    lines.append("| Reference | Fold mean | Fold min | Fold std | Full cal | Holdout |")
    lines.append("| --- | ---: | ---: | ---: | ---: | ---: |")
    for reference_id, raw in sorted(references.items()):
        row = _mapping(raw)
        stability = _mapping(row.get("fold_stability"))
        calibration = _mapping(row.get("calibration"))
        holdout = _mapping(row.get("holdout"))
        lines.append(
            f"| `{_escape(reference_id)}` | "
            f"`{_escape(stability.get('mean_score'))}` | "
            f"`{_escape(stability.get('min_score'))}` | "
            f"`{_escape(stability.get('score_std'))}` | "
            f"`{_escape(calibration.get('proficiency_ordering_score'))}` | "
            f"`{_escape(holdout.get('proficiency_ordering_score'))}` |"
        )
    lines.append("")
    return lines


def _leaderboard_section(
    report: Mapping[str, object],
    key: str,
    title: str,
    *,
    dataset_label: str,
) -> list[str]:
    rows = _mapping_rows(_mapping(report.get("leaderboards")).get(key))
    lines = [f"## {title}", ""]
    if not rows:
        lines.extend(["No rows.", ""])
        return lines
    lines.append(
        "| Rank | Candidate | Feature Set | Lane | Selector | Fold Mean | "
        "Fold Min | Fold Std | Full Cal | Optimism | Holdout | Holdout-Fold |"
    )
    lines.append(
        "| ---: | --- | --- | --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |"
    )
    for index, row in enumerate(rows, start=1):
        lines.append(
            f"| {index} | `{_escape(row.get('candidate_id'))}` | "
            f"`{_escape(row.get('feature_set_id'))}` | "
            f"`{_escape(row.get('lane_policy'))}` | "
            f"`{_escape(row.get('stability_selector_score'))}` | "
            f"`{_escape(row.get('fold_mean'))}` | "
            f"`{_escape(row.get('fold_min'))}` | "
            f"`{_escape(row.get('fold_std'))}` | "
            f"`{_escape(row.get('full_calibration_score'))}` | "
            f"`{_escape(row.get('full_calibration_optimism'))}` | "
            f"`{_escape(row.get('holdout_score'))}` | "
            f"`{_escape(row.get('holdout_delta_from_fold_mean'))}` |"
        )
    lines.append("")
    lines.append(f"Dataset scored in this table: `{dataset_label}`.")
    lines.append("")
    return lines


def _mean_metric(rows: Sequence[Mapping[str, object]], key: str) -> float | None:
    values = [_optional_float(row.get(key)) for row in rows]
    clean = [value for value in values if value is not None]
    if not clean:
        return None
    return float(sum(clean) / len(clean))


def _mapping_list_numbers(value: object) -> list[float]:
    if not isinstance(value, Sequence) or isinstance(value, (str, bytes)):
        return []
    result = []
    for raw in value:
        parsed = _optional_float(raw)
        if parsed is not None:
            result.append(parsed)
    return result


def _mean_number_list(values: Sequence[float]) -> float | None:
    if not values:
        return None
    return float(sum(values) / len(values))


def _float_mapping(value: object) -> dict[str, float]:
    result = {}
    for key, raw in _mapping(value).items():
        parsed = _optional_float(raw)
        if parsed is not None:
            result[str(key)] = parsed
    return result


def _mapping_rows(value: object) -> list[Mapping[str, object]]:
    if not isinstance(value, Sequence) or isinstance(value, (str, bytes)):
        return []
    return [row for row in value if isinstance(row, Mapping)]


def _resolve_path(path: Path) -> Path:
    return path if path.is_absolute() else PROJECT_ROOT / path


if __name__ == "__main__":
    raise SystemExit(main())
