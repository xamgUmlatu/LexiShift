...
DEFAULT_REVIEW_PACKET_JSON = TEST_OUTPUTS_ROOT / "srs_jmdict_topic_review_packet_en_ja_latest.json"
DEFAULT_OVERLAY_JSON_OUT = TEST_OUTPUTS_ROOT / "srs_jmdict_topic_overlay_en_ja_latest.json"
DEFAULT_JSON_OUT = TEST_OUTPUTS_ROOT / "srs_jmdict_topic_overlay_poc_en_ja_latest.json"
DEFAULT_MARKDOWN_OUT = TEST_OUTPUTS_ROOT / "srs_jmdict_topic_overlay_poc_en_ja_latest.md"
DEFAULT_PAIR = "en-ja"
DEFAULT_SOURCE_LABEL = "freq-ja-bccwj"
DEFAULT_TOP_N = 10000
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Build and exercise a diagnostic en-ja JMDict/BCCWJ topic-overlay PoC "
            "from reviewed topic labels. This does not mutate helper state or enable "
            "a product overlay."
        )
    )
...
def main() -> int:
    ...
    frequency_db = (
        ...
        else _input_path_from_review_packet(review_packet, key="frequency_db")
    )
    jmdict_path = (
        _resolve_path(args.jmdict)
        if args.jmdict
        else _input_path_from_review_packet(review_packet, key="jmdict")
    )
    report = build_report(
...
def build_report(
    *,
    ...
    resolved_jmdict_path = (
        Path(jmdict_path).expanduser().resolve(strict=False) if jmdict_path else None
    )
    missing_findings = _resource_findings(
        frequency_db=resolved_frequency_db,
        jmdict_path=resolved_jmdict_path,
    )
    if any(row["level"] == "FAIL" for row in missing_findings):
        return _report(
            status="review",
            generated_at=generated_at,
            review_packet_path=review_packet_path,
            ...
            findings=[
                *missing_findings,
                _finding(
                    "FAIL",
                    "seed_frontier_not_loaded",
                    "Missing resources prevented the admission PoC from loading seeds.",
                ),
            ],
        )
    ...
    scenarios = [
        ...
        for interest in _profile_interests_from_overlay(overlay)
    ]
    findings = _findings(
        overlay=overlay,
        seeds=seeds,
        scenarios=scenarios,
        resource_findings=missing_findings,
    )
...
def build_topic_overlay(
    *,
    ...
    for row in _mapping_rows(review_packet_payload.get("review_queue")):
        ...
        topic = str(row.get("family_id") or row.get("family") or "").strip()
        ...
        overlay_row = {
            ...
            "primary_source_label": str(row.get("primary_source_label") or ""),
            "evidence": {
                "rank": _round_float(row.get("rank")),
                "pos": str(row.get("pos") or ""),
                "pos_bucket": str(row.get("pos_bucket") or ""),
                "jmdict_match_modes": _string_list(row.get("jmdict_match_modes")),
                "jmdict_matched_terms": _string_list(row.get("jmdict_matched_terms")),
            },
            "provenance": {
                "review_packet": _repo_path(review_packet_path),
                "label_source": str(manual_review.get("label_source") or ""),
                "promotion_state": _promotion_state(review_packet_payload),
                "notes": str(manual_review.get("notes") or ""),
            },
        }
        key = (lemma, topic)
        existing = rows_by_key.get(key)
        if existing is None or float(overlay_row["membership"]) > float(existing["membership"]):
    ...
    if rows:
        findings.append(
            _finding(
                "PASS",
                "overlay_rows_present",
                "Accepted en-ja topic review labels produced overlay rows.",
            )
        )
    else:
        findings.append(
            _finding(
                "FAIL",
                "overlay_rows_empty",
                "No accepted en-ja topic review labels produced overlay rows.",
            )
        )
    if unlabeled_count:
        findings.append(
    ...
    artifact_ready = not any(item["level"] == "FAIL" for item in findings)
    return {
        "schema_version": 1,
        "overlay_id": _overlay_id_for_promotion_state(promotion_state),
        "status": "ok" if artifact_ready else "review",
        "decision": (
            _overlay_decision_for_promotion_state(promotion_state)
            if artifact_ready
        ...
        "inputs": {
            ...
            "labels_state": str(
                _as_mapping(review_packet_payload.get("label_result")).get("labels_state") or ""
            ),
        },
        "overlay_policy": {
            ...
            "strong_labels_runtime_effective": True,
            "light_labels_runtime_effective": False,
            "rejected_decisions_excluded": dict(sorted(rejected_counts.items())),
        },
        "summary": {
            "row_count": len(rows),
            "runtime_effective_row_count": sum(
                1
                for row in rows
                if (_round_float(row.get("membership")) or 0.0) >= PROFILE_INJECTION_MIN_MEMBERSHIP
            ),
...
def _report(
    *,
    ...
    return {
        ...
        "status": status,
        "decision": (
            "srs_jmdict_topic_overlay_poc_ready"
            if status == "ok"
            else "srs_jmdict_topic_overlay_poc_needs_review"
        ),
        "generated_at": generated_at,
        "inputs": {
            "review_packet_json": _repo_path(review_packet_path),
            "frequency_db": str(frequency_db or ""),
            "jmdict": str(jmdict_path or ""),
            "pair": pair,
            "source_label": source_label,
            "top_n": int(top_n),
        ...
        "methodology": {
            ...
            "overlay_application": (
                "Accepted review labels are converted to an approved candidate overlay. "
                "Only strong accepted labels are injected into seed profile_topics, "
                "then existing profile-bootstrap reranking is run per supported topic."
            ),
            "profile_injection_min_membership": PROFILE_INJECTION_MIN_MEMBERSHIP,
        },
...
def render_markdown(report: Mapping[str, object]) -> str:
    ...
    lines = [
        ...
        f"- JMDict: `{_as_mapping(report.get('inputs')).get('jmdict', '')}`",
        f"- Overlay rows: `{overlay_summary.get('row_count', 0)}`",
        f"- Overlay topics: `{overlay_summary.get('counts_by_topic', {})}`",
        f"- Overlay confidence: `{overlay_summary.get('counts_by_confidence', {})}`",
        "",
        "## Findings",
        "",
    ]
    ...
    scenarios = [_as_mapping(row) for row in report.get("profile_scenarios", [])]
    if scenarios:
...
def _scenario_markdown(scenario: Mapping[str, object]) -> list[str]:
    ...
    for row in _mapping_rows(with_overlay.get("top_rows")):
        lines.append(
            ...
            f"`{', '.join(_string_list(row.get('topic_hints'))[:8])}` | "
            f"`{', '.join(_string_list(row.get('overlay_topics'))[:8])}` | "
            f"{row.get('topic_affinity', '')} | "
            f"`{row.get('topic_affinity_source', '')}` |"
        )
    lines.append("")
    return lines
...
def _findings(
    *,
    ...
    seeds: Sequence[object],
    scenarios: Sequence[Mapping[str, object]],
    resource_findings: Sequence[Mapping[str, object]],
) -> list[dict[str, object]]:
    findings = [dict(row) for row in resource_findings]
    if str(overlay.get("status") or "") == "ok":
        findings.append(
            _finding("PASS", "topic_overlay_ready", "en-ja topic overlay candidate was built.")
        )
    else:
        findings.append(
            _finding("FAIL", "topic_overlay_not_ready", "en-ja topic overlay needs review.")
        )
    if seeds:
    ...
    if scenarios and any(_scenario_delta(row) > 0 for row in scenarios):
        findings.append(
            _finding(
                "PASS",
                "overlay_lifts_at_least_one_profile",
                "Overlay increases topic rows for at least one profile scenario.",
            )
        )
    elif scenarios:
        findings.append(
            _finding(
                "WARN",
                "overlay_profile_delta_thin",
                "Overlay did not increase topic rows in the top preview.",
            )
        )
    else:
        findings.append(
            _finding(
                "WARN",
                "overlay_profile_scenarios_empty",
                "No strong accepted topic labels were available for profile movement.",
            )
        )
    for scenario in scenarios:
        interest = str(scenario.get("interest") or "")
        ...
        findings.append(
            _finding(
                ...
                (
                    f"Overlay increases {interest} rows in the profile preview."
                    if delta > 0
                    else f"Overlay does not increase {interest} rows in the top preview."
                ),
            )
        )
    return findings
...
def _summary(
    *,
    ...
    return {
        ...
        "overlay_counts_by_confidence": overlay_summary.get("counts_by_confidence", {}),
        "overlay_review_state_counts": overlay_summary.get("review_state_counts", {}),
        "scenario_count": len(scenarios),
        "scenario_deltas": {
            str(row.get("interest") or ""): _as_mapping(row.get("delta"))
            for row in scenarios
            if str(row.get("interest") or "")
        },
...
def _promotion_state(review_packet_payload: Mapping[str, object]) -> str:
    label_state = str(
        _as_mapping(review_packet_payload.get("label_result")).get("labels_state") or ""
    )
    if label_state == "approved_by_user":
        return "approved_product_candidate_not_default"
    return "poc_candidate_not_product_overlay"


def _overlay_id_for_promotion_state(promotion_state: str) -> str:
    if promotion_state == "approved_product_candidate_not_default":
        return "srs_jmdict_topic_overlay_en_ja_approved_candidate_v1"
    return "srs_jmdict_topic_overlay_en_ja_poc_v1"


def _overlay_decision_for_promotion_state(promotion_state: str) -> str:
    if promotion_state == "approved_product_candidate_not_default":
        return "topic_overlay_approved_candidate_ready"
    return "topic_overlay_candidate_ready"


def _profile_interests_from_overlay(overlay: Mapping[str, object]) -> tuple[str, ...]:
    topics = [
        str(row.get("topic") or "")
        for row in _mapping_rows(overlay.get("rows"))
        if (_round_float(row.get("membership")) or 0.0) >= PROFILE_INJECTION_MIN_MEMBERSHIP
    ]
    return tuple(sorted(dict.fromkeys(topic for topic in topics if topic)))


def _seed_with_overlay(seed: object, overlay_rows: Sequence[Mapping[str, object]]) -> object:
    if not overlay_rows:
        return seed
...
def _resource_findings(
    *,
    ...
    return [
        _finding(
            "PASS" if frequency_db and frequency_db.exists() else "FAIL",
            "frequency_db_present",
            f"Frequency DB: {frequency_db or ''}",
        ),
        _finding(
            "PASS" if jmdict_path and jmdict_path.exists() else "FAIL",
            "jmdict_present",
            f"JMDict: {jmdict_path or ''}",
        ),
    ]


def _input_path_from_review_packet(
    review_packet_payload: Mapping[str, object],
    *,
    key: str,
) -> Path | None:
    ...
    raw_value = str(_as_mapping(audit_payload.get("inputs")).get(key) or "")
    return _resolve_path(Path(raw_value)) if raw_value else None
...
def _scenario_delta(scenario: Mapping[str, object]) -> int:
    return int(_as_mapping(scenario.get("delta")).get("overlay_topic_rows_in_top_n") or 0)
