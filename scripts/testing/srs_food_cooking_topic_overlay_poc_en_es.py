...
DEFAULT_REVIEW_PACKET_JSON = (
    ...
    / "docs"
    / "test_outputs"
    / "srs_food_cooking_signal_review_packet_en_es_current_latest.json"
)
...
DEFAULT_JSON_OUT = (
    ...
    / "docs"
    / "test_outputs"
    / "srs_food_cooking_topic_overlay_poc_en_es_current_latest.json"
)
DEFAULT_MARKDOWN_OUT = (
    PROJECT_ROOT
    / "docs"
    / "test_outputs"
    / "srs_food_cooking_topic_overlay_poc_en_es_current_latest.md"
)
DEFAULT_SOURCE_LABEL = "freq-es-cde"
DEFAULT_PAIR = "en-es"
DEFAULT_TOP_N = 10000
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Build and exercise an en-es food/cooking topic-overlay PoC from reviewed "
            "current-CDE labels. Diagnostic-only; no helper state mutation."
        )
    )
...
def build_report(
    *,
        ...
        return _report(
            ...
            findings=[
                _finding(
                    ...
                    "Frequency DB is missing; overlay was built but profile PoC could not run.",
                )
            ],
        )
    ...
    findings = _findings(overlay=overlay, seeds=seeds, scenario=scenario)
...
def build_topic_overlay(
    *,
    ...
    for row in _mapping_rows(review_packet_payload.get("review_queue")):
        ...
        topic = str(row.get("family") or "").strip() or TOPIC
        if not lemma or topic != TOPIC:
            continue
        overlay_row = {
            "lemma": lemma,
            "language_pair": pair,
        ...
        rows_by_key[(lemma, topic)] = overlay_row
    ...
    if rows:
        findings.append(
            _finding(
                ...
                "Accepted food/cooking review labels produced overlay rows.",
            )
        )
    else:
        findings.append(
            _finding(
                "FAIL",
                "overlay_rows_empty",
                "No accepted food/cooking review labels produced overlay rows.",
            )
        )
    if unlabeled_count:
        findings.append(
    ...
    return {
        "schema_version": 1,
        "overlay_id": "srs_food_cooking_topic_overlay_en_es_current_poc_v1",
        ...
        "summary": {
            "row_count": len(rows),
            "counts_by_topic": {TOPIC: len(rows)} if rows else {},
...
def _report(
    *,
    ...
    return {
        ...
        if status == "ok"
        else "srs_food_cooking_topic_overlay_poc_needs_review",
        "generated_at": generated_at,
        "inputs": {
            "review_packet_json": _repo_path(review_packet_path),
        ...
        "methodology": {
            ...
            "overlay_application": (
                ...
                "then existing profile-bootstrap reranking is run for a diagnostic food profile. "
                "Only strong rows are injected; light rows remain in the overlay artifact."
            ),
            "profile_injection_min_membership": PROFILE_INJECTION_MIN_MEMBERSHIP,
        },
        "topic_overlay": overlay,
        "profile_scenario": dict(scenario),
        "findings": list(findings),
        "summary": _summary(overlay=overlay, scenario=scenario, findings=findings),
        "limitations": [
...
def render_markdown(report: Mapping[str, object]) -> str:
    ...
    scenario = _as_mapping(report.get("profile_scenario"))
    lines = [
...
def _findings(
    *,
    ...
    if str(overlay.get("status") or "") == "ok":
        findings.append(
            _finding("PASS", "topic_overlay_ready", "Food/cooking overlay candidate was built.")
        )
    else:
        findings.append(
            _finding(
                "FAIL",
                "topic_overlay_not_ready",
                "Food/cooking overlay candidate needs review.",
            )
        )
    if seeds:
    ...
    if int(delta.get("overlay_topic_rows_in_top_n") or 0) > 0:
        findings.append(
            _finding(
                "PASS",
                "overlay_lifts_profile:food_cooking",
                "Overlay increases food/cooking rows in the profile preview.",
            )
        )
    else:
        findings.append(
            _finding(
                "WARN",
                "overlay_profile_delta_thin:food_cooking",
                "Overlay did not increase food/cooking rows in the top preview.",
            )
        )
    return findings
...
def _summary(
    *,
    ...
    return {
        ...
        "overlay_counts_by_confidence": _as_mapping(overlay.get("summary")).get(
            "counts_by_confidence", {}
        ),
        "scenario_delta": _as_mapping(scenario.get("delta")),
