...
DEFAULT_LEXICON_JSON = TEST_INPUTS_ROOT / "srs_topic_manual_semantic_lexicon_en_de.json"
DEFAULT_FREQUENCY_DB = DEFAULT_DATA_ROOT / "frequency_packs" / "freq-de-default.sqlite"
DEFAULT_JSON_OUT = TEST_OUTPUTS_ROOT / "srs_topic_manual_semantic_lexicon_en_de_latest.json"
DEFAULT_MARKDOWN_OUT = TEST_OUTPUTS_ROOT / "srs_topic_manual_semantic_lexicon_en_de_latest.md"
LANGUAGE_PAIR = "en-de"
SOURCE_CHANNEL = "product_owned_manual_semantic_lexicon"
OVERLAY_ID = "srs_topic_manual_semantic_lexicon_en_de_v1"


def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Build a product-owned en-de topic overlay from conservative manual "
            "semantic seed lists. Entries are matched to the German SRS corpus after "
            "corpus-form normalization for German capitalization."
        )
    )
    ...
    *,
    ...
    for collection in _mapping_rows(lexicon.get("collections")):
        ...
        for entry in _entry_rows(collection.get("entries")):
            ...
            if not lemma:
                continue
            frequency = _resolve_frequency_row(frequency_rows, lemma)
    ...
    return {
        ...
        "overlay_policy": {
            ...
            "match_policy": "exact_or_lowercase_lemma_in_frequency_corpus",
...
def render_markdown(report: Mapping[str, object]) -> str:
    ...
    lines = [
        "# en-de Manual Semantic Topic Lexicon",
        "",
...
def _overlay_row(
    *,
    ...
    lemma = str(frequency.get("lemma") or entry.get("lemma") or "").strip()
    original_lemma = str(entry.get("lemma") or "").strip()
    collection_id = str(collection.get("id") or "").strip()
...
def _resolve_frequency_row(
    ...
    stripped = str(lemma or "").strip()
    if not stripped:
        return None
    exact = frequency_rows.get(stripped)
    if exact is not None:
        return exact
    lower = stripped.lower()
    if lower != stripped:
        return frequency_rows.get(lower)
    return None
...
def _review_id(collection_id: str, lemma: str, topic: str) -> str:
    ...
    return f"srs-ende-manual-topic-{digest}"
