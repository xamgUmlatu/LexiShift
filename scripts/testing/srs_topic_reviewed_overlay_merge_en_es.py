...
DEFAULT_JSON_OUT = TEST_OUTPUTS_ROOT / "srs_topic_reviewed_overlay_merged_en_es_latest.json"
DEFAULT_MARKDOWN_OUT = TEST_OUTPUTS_ROOT / "srs_topic_reviewed_overlay_merged_en_es_latest.md"
LANGUAGE_PAIR = "en-es"
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Merge reviewed en-es topic overlay artifacts into one preferred runtime "
    ...
    parser.add_argument(
        ...
        type=Path,
        default=[],
        help="Reviewed topic overlay JSON. May be repeated. Defaults to the current en-es overlay stack.",
    )
...
def build_overlay(
    *,
    ...
    return {
        "schema_version": 1,
        "status": "ok",
        "decision": "srs_topic_reviewed_overlay_merged_en_es_ready",
        "generated_at": generated_at or _utc_now(),
        "language_pair": LANGUAGE_PAIR,
        "overlay_id": "srs_topic_reviewed_overlay_merged_en_es_v1",
        "overlay_policy": {
...
def render_markdown(report: Mapping[str, object]) -> str:
    ...
    lines = [
        "# en-es Reviewed Topic Overlay Merge",
        "",
...
def _row_priority(row: Mapping[str, object]) -> tuple[float, int, float]:
    membership = _safe_float(row.get("membership"))
    confidence = str(row.get("confidence_label") or "").strip()
    confidence_score = {"strong": 2, "light": 1}.get(confidence, 0)
    evidence_score = _safe_float(row.get("evidence_score"))
    return membership, confidence_score, evidence_score


def _summary(rows: Sequence[Mapping[str, object]]) -> dict[str, object]:
    raw_counts = Counter(str(row.get("topic") or "") for row in rows if row.get("topic"))
    runtime_rows = [row for row in rows if _safe_float(row.get("membership")) >= 1.0]
    runtime_counts = Counter(
        str(row.get("topic") or "") for row in runtime_rows if row.get("topic")
    )
    confidence_counts = Counter(str(row.get("confidence_label") or "unknown") for row in rows)
    runtime_confidence_counts = Counter(
        str(row.get("confidence_label") or "unknown") for row in runtime_rows
    )
    return {
        "row_count": len(rows),
        "runtime_effective_row_count": len(runtime_rows),
        "topic_count": len(raw_counts),
        "runtime_effective_topic_count": len(runtime_counts),
        "counts_by_topic": dict(sorted(raw_counts.items())),
        ...
        "runtime_effective_counts_by_confidence": dict(sorted(runtime_confidence_counts.items())),
    }
