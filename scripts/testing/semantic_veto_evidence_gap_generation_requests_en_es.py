...
def _prompt_text(request: Mapping[str, object]) -> str:
    ...
    if slot_type == "active_evidence_expansion":
        ...
        task = (
            ...
            "is provided, use that competitor sense. If blank, propose one plausible "
            "Spanish competitor target only when it meets that distinct-sense test. Do not "
            "use near-synonyms, stylistic variants, grammatical meta-examples, or examples "
            "that still fit active_evidence_text. If no clearly distinct competitor exists, "
            "return unable_to_find_distinct_competitor:true with empty items and explain "
            "why. When generating items, include a competitor_sense_label and "
            ...
            "active_mismatch_note that names active_target_lemma exactly and explains why "
            "that active target is wrong in the generated sentence."
        )
        schema = (
            "Schema: {request_id, family_id, slot_id, slot_type, source_phrase, "
        ...
        task = (
            ...
            "known competitor target should be inserted. Good contexts include page titles, "
            "visible headings, UI labels, table labels, quoted user text, or literal search "
            "queries when natural. Avoid filenames, file paths, URLs, code identifiers, "
            "underscores, embedded-token examples, and ordinary sentences that simply use "
            "the active target sense. Each item must set no_winner_context_class to one of "
            "page_title, visible_heading, ui_label, table_label, quoted_user_text, "
            "search_query, name_or_title, or literal_fragment. Each item must explain both "
            "why the replacement should not happen and why the English source token is "
            "still a runtime-like standalone match."
        )
        schema = (
            "Schema: {request_id, family_id, slot_id, slot_type, source_phrase, "
