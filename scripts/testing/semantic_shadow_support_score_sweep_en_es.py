...
DEFAULT_JSON_OUT = (
    PROJECT_ROOT / "docs" / "test_outputs" / "semantic_shadow_support_score_sweep_en_es_latest.json"
)
DEFAULT_MARKDOWN_OUT = (
    PROJECT_ROOT / "docs" / "test_outputs" / "semantic_shadow_support_score_sweep_en_es_latest.md"
)
DEFAULT_MODE_IDS = (
    "benchmark_reviewed",
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Sweep the en-es shadow support-score threshold and max-promoted count "
            "against the reviewed-trigger overlap gold proxy."
        )
    )
    parser.add_argument(
        "--benchmark-dataset",
...
def build_support_score_sweep_report(
    *,
    ...
    gold_reference = compare_report.get("gold_reference")
    ...
    for mode_id in mode_ids:
        ...
        if isinstance(proxy, Mapping):
            ...
            if isinstance(policies, Mapping):
                baseline = policies.get("cross_checked_v1")
                ...
                rows.append(
                    {
                        ...
                        "baseline_cross_checked_precision": baseline_summary.get(
                            "candidate_precision"
                        ),
                        "baseline_cross_checked_recall": baseline_summary.get("candidate_recall"),
                        "baseline_cross_checked_overblocking": baseline_summary.get(
                            "overblocking_rate"
                        ),
                    }
                )
    ...
    return {
        ...
        "gold_reference": gold_reference if isinstance(gold_reference, Mapping) else {},
...
def _render_markdown(report: Mapping[str, object]) -> str:
    lines = [
        ...
        (
            "- Sweep meaning: keep candidate mining fixed per seed mode, then vary only the "
            "support-score threshold and the maximum number of promoted shadows."
        ),
    ]
        ...
        lines.append(
            ...
            + " | ".join(
                [
                    ...
                    _render_rate(row.get("baseline_cross_checked_precision")),
                    _render_rate(row.get("baseline_cross_checked_recall")),
                    _render_rate(row.get("baseline_cross_checked_overblocking")),
                ]
            )
            + " |"
        )
    ...
    if isinstance(best_rows, Mapping) and best_rows:
        ...
        for mode_id, row in best_rows.items():
            ...
            lines.append(
                "- "
                f"`{mode_id}` with `min_score={row.get('support_score_min')}` and "
...
def main() -> int:
    args = _parse_args()
    report = build_support_score_sweep_report(
