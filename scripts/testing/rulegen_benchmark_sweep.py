...
def _build_sweep_configs(args: argparse.Namespace) -> list[SweepConfig]:
    ...
    interleave_definition_group_values = _parse_csv_bools(
        args.interleave_definition_group_values,
        name="interleave-definition-group-values",
    )
    sense_representative_selection_values = _parse_csv_bools(
        args.sense_representative_selection_values,
        name="sense-representative-selection-values",
    )
    ...
    exact_gloss_demotion_values = _parse_csv_bools(
        args.exact_gloss_demotion_values,
        name="exact-gloss-demotion-values",
    )
    source_frequency_prior_values = _parse_csv_bools(
        args.source_frequency_prior_values,
        name="source-frequency-prior-values",
    )
    cleaner_later_competition_penalty_values = _parse_csv_floats(
        args.cleaner_later_competition_penalty_values,
        name="cleaner-later-competition-penalty-values",
    )
    sense_defaultness_competition_penalty_values = _parse_csv_floats(
        args.sense_defaultness_competition_penalty_values,
        name="sense-defaultness-competition-penalty-values",
    )
    ...
    for combo in itertools.product(
        ...
        max_rules_values,
        interleave_definition_group_values,
        sense_representative_selection_values,
        confidence_values,
        semantic_demotion_scale_values,
        exact_gloss_demotion_values,
        source_frequency_prior_values,
        cleaner_later_competition_penalty_values,
        sense_defaultness_competition_penalty_values,
        ...
        configs.append(
            SweepConfig(
                ...
                interleave_definition_groups=bool(combo[2]),
                sense_representative_selection=bool(combo[3]),
                confidence_threshold=float(combo[4]),
                semantic_demotion_scale=float(combo[5]),
                exact_gloss_demotion_enabled=bool(combo[6]),
                source_frequency_prior_enabled=bool(combo[7]),
                cleaner_later_competition_penalty=float(combo[8]),
                sense_defaultness_competition_penalty=float(combo[9]),
                include_variants=bool(combo[10]),
                pos_scoring_enabled=bool(combo[11]),
                pos_exact_match_bonus=float(combo[12]),
                pos_compatible_match_bonus=float(combo[13]),
                score_weight_dict_priority=float(combo[14]),
                score_weight_frequency_weight=float(combo[15]),
                score_weight_pos_match=float(combo[16]),
                score_weight_variant_penalty=float(combo[17]),
                score_weight_phrase_penalty=float(combo[18]),
                score_weight_embedding=float(combo[19]),
                reverse_check_enabled=bool(combo[20]),
                reverse_check_match_bonus=float(combo[21]),
                reverse_check_near_bonus=float(combo[22]),
                reverse_check_near_rank_max=max(0, int(combo[23])),
                reverse_check_far_hit_penalty=float(combo[24]),
                reverse_check_miss_penalty=float(combo[25]),
                reverse_check_exact_hit_ambiguity_threshold=max(0, int(combo[26])),
                reverse_check_exact_hit_ambiguity_penalty=float(combo[27]),
                kaikki_policy_register_demotion=bool(combo[28]),
                kaikki_policy_live_demotion=bool(combo[29]),
                kaikki_policy_risk_families=tuple(combo[30]),
                reverse_check_exact_hit_specificity_bonus=float(combo[31]),
                kaikki_policy_late_sense_penalty=float(combo[32]),
            )
        )
    return configs
...
def _build_rulegen_adapter_request(
    *,
    ...
    return RulegenAdapterRequest(
        ...
        enable_exact_gloss_demotions=config.exact_gloss_demotion_enabled,
        enable_source_frequency_prior=config.source_frequency_prior_enabled,
