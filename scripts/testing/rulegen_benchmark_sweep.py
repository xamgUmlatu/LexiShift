    ...
    for combo in itertools.product(
        ...
        configs.append(
            SweepConfig(
                ...
                exact_gloss_demotion_enabled=bool(combo[4]),
                source_frequency_prior_enabled=bool(combo[5]),
                include_variants=bool(combo[6]),
                pos_scoring_enabled=bool(combo[7]),
                pos_exact_match_bonus=float(combo[8]),
                pos_compatible_match_bonus=float(combo[9]),
                score_weight_dict_priority=float(combo[10]),
                score_weight_frequency_weight=float(combo[11]),
                score_weight_pos_match=float(combo[12]),
                score_weight_variant_penalty=float(combo[13]),
                score_weight_phrase_penalty=float(combo[14]),
                score_weight_embedding=float(combo[15]),
                reverse_check_enabled=bool(combo[16]),
                reverse_check_match_bonus=float(combo[17]),
                reverse_check_near_bonus=float(combo[18]),
                reverse_check_near_rank_max=max(0, int(combo[19])),
                reverse_check_far_hit_penalty=float(combo[20]),
                reverse_check_miss_penalty=float(combo[21]),
                reverse_check_exact_hit_ambiguity_threshold=max(0, int(combo[22])),
                reverse_check_exact_hit_ambiguity_penalty=float(combo[23]),
                kaikki_policy_live_demotion=bool(combo[24]),
                kaikki_policy_risk_families=tuple(combo[25]),
                reverse_check_exact_hit_specificity_bonus=float(combo[26]),
                kaikki_policy_late_sense_penalty=float(combo[27]),
            )
        )
    return configs
