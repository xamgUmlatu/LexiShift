...
def _render_html_report(
    *,
    ...
    for pair, runs in sorted(pair_runs.items()):
        ...
        case_rows: list[str] = []
        for case in best_run.case_results:
            target = _escape_html(case.get("target", ""))
            case_id = _escape_html(case.get("case_id", ""))
            top1_source = _escape_html(case.get("top1_source", "-") or "-")
            top1_conf = _format_optional_float(case.get("top1_confidence"), digits=4)
            top3_sources_raw = case.get("top3_sources")
            if isinstance(top3_sources_raw, Sequence) and not isinstance(top3_sources_raw, (str, bytes)):
                ...
                top3_sources = "-"
            top1_correct = bool(case.get("top1_correct", False))
            top3_contains = bool(case.get("top3_contains_expected", False))
            top1_forbidden = bool(case.get("top1_forbidden", False))
            forbidden_any = bool(case.get("forbidden_any_present", False))
            variant_count = int(case.get("variant_rule_count", 0) or 0)
            rule_count = int(case.get("rule_count", 0) or 0)
        ...
        pair_sections.append(
            ...
            f"<div class=\"pair-head\"><h2>{_escape_html(pair)}</h2>"
            ...
            "</details>"
            "<details>"
            "<summary>Best Run Case Diagnostics</summary>"
            "<div class=\"table-wrap\"><table><thead><tr>"
    ...
    return "".join(
        [
            ...
            "@media (max-width:1020px){.metric-grid{grid-template-columns:repeat(3,minmax(140px,1fr));}}",
            "@media (max-width:640px){main{padding:20px 12px 36px}.metric-grid{grid-template-columns:repeat(2,minmax(120px,1fr));}",
