...
DEFAULT_PACK_ID = "freq-es-spalex-expanded-v1"
DEFAULT_PROVIDER = "freq-es-spalex-expanded-v1"
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Build a reversible en-es research frequency pack from the current "
            "freq-es-cde seed plus SPALEX-ranked additions."
        )
    )
    ...
    parser.add_argument("--current-frequency-db", type=Path, default=DEFAULT_CURRENT_FREQUENCY_DB)
...
def build_spalex_frequency_pack(
    *,
    ...
    current_rows = _load_current_frequency_rows(current_frequency_db)
    spalex_rows = _load_spalex_rows(spalex_csv)
    enrichments = _load_kaikki_enrichment(kaikki_forward_db) if kaikki_forward_db else {}
    combined_rows = _combine_rows(current_rows, spalex_rows, target_size=target_size)
    row_count = _write_frequency_sqlite(
        ...
        source_paths={
            ...
            "current_frequency_db": Path(current_frequency_db),
            "kaikki_forward_db": Path(kaikki_forward_db) if kaikki_forward_db else None,
        },
    )
...
def _load_kaikki_enrichment(path: Path) -> dict[str, KaikkiEnrichment]:
    ...
    for lemma, topics in topics_by_lemma.items():
...
def _combine_rows(
    ...
    *,
    ...
    for row in current_rows.values():
        seen.add(row.lemma)
        combined.append({"lemma": row.lemma, "source_family": "freq-es-cde"})
    for row in sorted(spalex_rows.values(), key=lambda item: item.rank):
        if row.lemma in seen:
            continue
        seen.add(row.lemma)
        combined.append({"lemma": row.lemma, "source_family": "spalex"})
...
def _write_sidecars(
    *,
    ...
    provenance = write_app_managed_pack_provenance(
        ...
        pack_kind="frequency",
        provider=provider,
        source_name="SPALEX plus freq-es-cde seed plus Kaikki enrichment",
        ...
        converter_version="build_spalex_frequency_pack_en_es_v1",
        parser_profile="spalex_cde_union_v1",
        parser_config={
            "current_seed": "freq-es-cde",
            "expansion_source": "spalex_word_info_csv",
            "rank_policy": "freq_es_cde_seed_then_spalex_zipf_prevalence",
            "runtime_pmw": "rank_descending_commonness_score",
            "pos_policy": "freq_es_cde_pos_else_kaikki_pos_mapped_to_compact_latin",
            "topic_policy": "kaikki_sense_topics_only",
        },
...
def _source_bundle(
    *,
    ...
    return {
        "bundle_id": f"{pack_id}:spalex_cde_kaikki_v1",
...
def _sqlite_metadata(
    *,
    ...
    return {
        ...
        "source_profile": "spalex_cde_union_v1",
        "row_count": row_count,
        "column_names": [
            "id",
            "pmw",
        ...
        "pos_policy": "freq_es_cde_compact_else_kaikki_compact_mapped",
        "topic_policy": "kaikki_sense_topics_csv",
        "sources": {
