...
class SrsLearnerDifficultyFirst500ReviewEnEsTests(unittest.TestCase):
    def test_applies_manual_correction_seed_without_changing_runtime(self) -> None:
        report = build_report(
        ...
        self.assertEqual(
            report["decision"],
            "en_es_learner_difficulty_first500_review_ready",
        )
        ...
        summary = report["summary"]["correction_summary"]
        self.assertEqual(summary["correction_rows"], 3)
        self.assertEqual(summary["active_count"], 3)
        self.assertEqual(summary["moved_count"], 3)

        rows = {row["lemma"]: row for row in report["first500_rows"]}
        self.assertEqual(rows["parte"]["model_score"], 0.05)
        self.assertEqual(rows["parte"]["effective_score"], 0.24)
        self.assertEqual(rows["hoy"]["effective_score"], 0.05)
        self.assertEqual(rows["son"]["effective_score"], 0.12)
        self.assertIn("restricted_admission", rows["son"]["review_flags"])
        self.assertEqual(rows["son"]["admission_override"], "inflected_or_nonlemma_form")

        markdown = render_markdown(report)
        self.assertIn("en-es First-500 Learner Difficulty Review", markdown)
        self.assertIn("Correction Applications", markdown)
        self.assertIn("restricted_admission", markdown)
...
def _override(
    ...
    *,
    ...
    admission_override: str | None = None,
) -> dict[str, object]:
    return {
        "lemma": lemma,
        "status": "active",
