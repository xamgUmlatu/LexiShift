...
try:
    ...
    _normalize_pos_fn: Callable[..., object] | None = _normalize_pos_impl
except Exception:  # noqa: BLE001
    _normalize_pos_fn = None
    ...
    *,
    ...
    try:
        ...
        metadata = {
            ...
            "pos_mapping_profile": "freedict" if _normalize_pos_fn is not None else None,
            "pos_mapping_available": bool(_normalize_pos_fn is not None),
...
def _is_pos_unmapped(raw_tag: str) -> bool:
    if _normalize_pos_fn is None:
        return False
    normalized = _normalize_pos_fn(
        raw_tag,
        source_provider="freedict",
        source_kind="dictionary",
        source_profile="freedict",
    )
    return not bool(getattr(normalized, "mapped", None))
