...
- `[~]` Persist browsing admission aggregate state (core decayed/bounded store and opt-in helper dev ingest exist; live extension capture is not wired).
...
- `[~]` Explicit policy gate for any non-feedback signals (browsing dev ingest requires opt-in; production capture policy is not wired).
