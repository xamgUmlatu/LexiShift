...
summaries, and can locally search/filter/sort the already-loaded payload.
Eligible rows expose a confirmed Discard action that reuses the existing
`srs_admission_suppress` route with `reason=user_blocked`.
