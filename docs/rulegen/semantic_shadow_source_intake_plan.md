...
- `docs/test_outputs/semantic_llm_example_frame_balanced_remediation_generalization_probe_latest.md` is now the broader safety read: the surface-POS guard still has `0` harmful replacements across the full `95`-case `v10` dataset and records `5` correct active rescues, but all-family recall remains low because the generated source batch only covers the frozen queue families
- `scripts/testing/semantic_llm_example_frame_remediation_plan_en_es.py` now defaults to the best remediation-eligible prototype guard instead of the stale phrase-containment config; against the latest balanced prototype output, `docs/test_outputs/semantic_llm_example_frame_plant_remediation_plan_latest.md` correctly narrows the next live source plan to `1` active `plant` request
- `scripts/testing/semantic_llm_example_frame_leakage_audit_en_es.py` now canonicalizes person/determiner substitutions when checking benchmark-near-copy spans, so `I/my` rewrites of a reviewed sentence no longer slip through admission
- `docs/test_outputs/semantic_llm_example_frame_plant_remediation_run_latest.md` and `docs/test_outputs/semantic_llm_example_frame_plant_remediation_v2_run_latest.md` preserve two bounded live plant attempts; both accepted structurally, but `docs/test_outputs/semantic_llm_example_frame_plant_remediation_leakage_audit_latest.md` and `docs/test_outputs/semantic_llm_example_frame_plant_remediation_v2_leakage_audit_latest.md` reject them as canonical benchmark-near-copies, leaving `0` admitted plant rows and no quality-gate improvement
