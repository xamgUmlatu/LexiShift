...
- `docs/test_outputs/semantic_non_v10_source_support_conversion_wave6_anypos_latest.md` quantifies that conversion gap. The selected upper-bound wave has `0 / 16` fully supported families and only `13 / 38` selected senses already supported; all `16` families need reviewed source support, with unsupported rows split across `10` active senses and `15` shadow senses. This makes reviewed/reverse-side source support the next high-leverage implementation path before any broad quality claim.
...
8. Use the wave6 supported-vs-upper-bound split as the next source-support conversion target: preserve the supported-lane `6 / 16` read as the conservative control, then add or ingest reviewed/reverse-side support for the selected forward-only families before any promotion claim.
9. After source support is added, rerun source extraction, source admission, the support-conversion audit, and fresh independent active/shadow plus phrase/no-winner held-out rows for the wave6 selected families before comparing against wave5.
