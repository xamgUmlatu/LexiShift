...
8. Treat the Wiktextract translation-table overlay as the current wave6 source-support conversion path: it closes the `0 / 16` supported-family gap and reruns WordNet source admission to `16 / 16` semantic coverage, but remains analysis-only until independent held-out validation exists.
9. Add fresh independent active/shadow plus phrase/no-winner held-out rows for the wave6 selected families, then run held-out validation, margin sweep, and failure-class mining before comparing wave6 against wave5.
