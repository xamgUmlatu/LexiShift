...
- `docs/test_outputs/semantic_source_non_v10_wave5_portfolio_heldout_margin005_validation_latest.md` and `docs/test_outputs/semantic_source_non_v10_wave5_portfolio_phrase_margin005_validation_latest.md` add those first independent held-out rows for the materialized portfolio. They correctly block promotion: the active/shadow suite has `2` harmful replacements and `3` false abstains across `32` cases, and the phrase/no-winner suite has `3` harmful replacements across `16` cases. A no-write margin sensitivity check did not find a clean threshold fix: margins `0` through `0.02` keep the same active/shadow failures, `0.08+` removes active/shadow harmful replacements only by increasing false abstains to `4`, and phrase still has `2` harmful replacements at `0.08+`. Treat this as source/POS/phrase blocker evidence, not a reason to tune on the held-out cases.
