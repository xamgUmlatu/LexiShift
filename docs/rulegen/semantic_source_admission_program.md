...
- `scripts/testing/semantic_wordnet_phrase_control_miner_en_es.py`
...
- `docs/test_outputs/semantic_wordnet_phrase_control_non_v10_wave5_portfolio_latest.md`
- `docs/test_outputs/semantic_source_admission_cycle_non_v10_wave5_phrase_probe_latest.md`
- `docs/test_outputs/semantic_source_non_v10_wave5_portfolio_heldout_margin005_validation_latest.md`
- `docs/test_outputs/semantic_source_non_v10_wave5_portfolio_phrase_margin005_validation_latest.md`
- `docs/test_outputs/semantic_source_margin_policy_sweep_non_v10_wave5_portfolio_latest.md`
- `docs/test_outputs/semantic_source_failure_class_mining_non_v10_wave5_portfolio_latest.md`
...
- `docs/test_outputs/semantic_non_v10_source_support_conversion_wave6_anypos_latest.md`
- `docs/test_outputs/semantic_wiktextract_translation_support_wave6_anypos_latest.md`
- `docs/test_outputs/semantic_non_v10_source_support_conversion_wave6_wiktextract_supported_latest.md`
- `docs/test_outputs/semantic_wiktextract_non_v10_wave6_upper_bound_latest.md`
- `docs/test_outputs/semantic_source_admission_cycle_wiktextract_non_v10_wave6_upper_bound_latest.md`
- `docs/test_outputs/semantic_wordnet_def_ex_non_v10_wave6_wiktextract_supported_latest.md`
- `docs/test_outputs/semantic_source_admission_cycle_wordnet_def_ex_non_v10_wave6_wiktextract_supported_latest.md`
- `docs/test_outputs/semantic_source_non_v10_wave6_wiktextract_supported_heldout_margin005_validation_latest.md`
- `docs/test_outputs/semantic_source_non_v10_wave6_wiktextract_supported_phrase_margin005_validation_latest.md`
- `docs/test_outputs/semantic_wordnet_phrase_control_non_v10_wave6_wiktextract_supported_latest.md`
- `docs/test_outputs/semantic_source_margin_policy_sweep_non_v10_wave6_wiktextract_supported_latest.md`
- `docs/test_outputs/semantic_source_failure_class_mining_non_v10_wave6_wiktextract_supported_latest.md`
...
- `docs/test_outputs/semantic_non_v10_source_portfolio_wave5_anypos_latest.md` materializes the source portfolio as one variant-aligned source batch instead of trusting the sweep's mixed embedded family payloads. The first attempt exposed a real `rest:reposo` payload/row mismatch; after the materializer rebuilt each selected family from the same supporting variant that supplied its rows, `docs/test_outputs/semantic_source_admission_cycle_non_v10_wave5_source_portfolio_latest.md` passes with `0` leakage rejects, `0` sense rejects, `51` final admitted rows, and semantic contract coverage `16 / 16`. That original portfolio remains useful as the semantic source control; phrase containment started as `0 / 16` before the separate phrase-control probe below.
- `docs/test_outputs/semantic_source_non_v10_wave5_portfolio_heldout_margin005_validation_latest.md` and `docs/test_outputs/semantic_source_non_v10_wave5_portfolio_phrase_margin005_validation_latest.md` add the first independent held-out rows for the materialized portfolio. The first read correctly blocked promotion at `2` harmful replacements / `3` false abstains on the `32`-case active/shadow suite and `3` harmful replacements on the `16`-case phrase/no-winner suite; a no-write margin sensitivity check showed this was not a simple threshold issue. A syntax candidate then improved the frozen suites to `1` harmful / `0` false abstains on active/shadow and `1` harmful / `0` false abstains on phrase/no-winner by exposing local trigger context and limiting modifier rescue to near ties.
- The next repair split the two remaining failures by data contract. `present:002` (`The present is all we can control`) is now gold `replace`, not gold abstain, because the source portfolio already contains the same visible target `presente` for the current-time noun sense; the user-visible replacement is therefore correct under the binary UX contract. The wave builder and WordNet batch adapter now preserve same-visible active alias evidence so this class is explicit rather than hidden in the held-out label.
- `docs/test_outputs/semantic_wordnet_phrase_control_non_v10_wave5_portfolio_latest.md` adds a failure-analysis WordNet phrase-control miner. It mines `7` containment-only rows across `7 / 16` phrase cases, including `end tables placed conveniently` for `end:001` (`The end table held a small lamp`). `docs/test_outputs/semantic_source_admission_cycle_non_v10_wave5_phrase_probe_latest.md` admits those rows with `0` leakage rejects and `0` sense rejects, raising phrase contract coverage to `7 / 16` while keeping the combined contract in `review`.
- With the same-visible alias correction and phrase-expanded evidence batch, the refreshed held-out artifacts now pass the frozen wave5 active/shadow suite and phrase/no-winner suite at `0` harmful replacements / `0` false abstains. This is real mechanism progress, but it is not a runtime-promotion claim: the phrase-control rows were mined against the same phrase suite as a discovery probe, the phrase suite has an empty-baseline abstain-all comparator, and fresh locked phrase/no-winner evaluation is still required.
- `docs/test_outputs/semantic_source_margin_policy_sweep_non_v10_wave5_portfolio_latest.md` reruns the margin-policy question after the phrase probe. Margins `0` through `0.02` pass both wave5 suites; higher margins introduce active false abstains for `rest:001` and then `present:002`. Treat `0` as the smallest current-suite candidate, not as a runtime default.
- `docs/test_outputs/semantic_source_failure_class_mining_non_v10_wave5_portfolio_latest.md` is the current wave5 anti-overfitting ledger. It reports `review` / `seed_pass_expand_inventory` with no blocking semantic failure classes, active/shadow semantic contract coverage at `16 / 16`, final admitted rows increased to `58`, phrase coverage at `7 / 16`, and a remaining breadth gap of `34` more families plus `152` more cases before the default broad-confidence thresholds. Use this report as the next-step source of truth: broaden inventory and held-out coverage while tracking the `9` remaining phrase-contract gaps (`dry`, `look`, `mean`, `offer`, `plain`, `present`, `quiet`, `sign`, `train`).
- `docs/test_outputs/semantic_non_v10_inventory_candidates_wave6_available_latest.md` starts the next breadth step by excluding the v10 inventory, seed non-v10 queue, selected wave2 families, and wave5 source-portfolio families. It still finds `100` ranked WordNet candidates after `43` existing triggers are excluded, so inventory availability is not the immediate ceiling.
- `docs/test_outputs/semantic_non_v10_wave_admission_sweep_wave6_anypos_supported_latest.md` runs the same supported-source bakeoff on that fresh candidate list. The supported lane now reaches only `6 / 16` admission-selected families (`19` final admitted rows), with the best variant leaving `split:partido` as a semantic gap. This is evidence that reverse/FreeDict-supported source rows are thinning out after the earlier waves.
- `docs/test_outputs/semantic_non_v10_wave_admission_sweep_wave6_anypos_unsupported_latest.md` runs the paired forward-only upper-bound sweep on the same candidate list. It finds a strong ceiling: `55 / 64` semantic coverage, `221` final admitted rows, and an admission-selected `16 / 16` control wave. That separates the bottleneck from the scoring/admission harness: candidate families and WordNet evidence exist, but many selected translations lack promotion-grade source support.
- `docs/test_outputs/semantic_non_v10_source_support_conversion_wave6_anypos_latest.md` quantifies that conversion gap and now emits an acquisition worklist. The selected upper-bound wave has `0 / 16` fully supported families and only `13 / 38` selected senses already supported; all `16` families need reviewed source support, with unsupported rows split across `10` active senses and `15` shadow senses. The worklist orders active gaps first, same-POS supported-target swaps second, and remaining shadow gaps third, with acceptable source lanes and admission checks attached to each row. This makes reviewed/reverse-side source support the next high-leverage implementation path before any broad quality claim.
- `docs/test_outputs/semantic_wiktextract_translation_support_wave6_anypos_latest.md` executes the first worklist conversion lane instead of leaving the queue manual. It scans the local raw Wiktextract English translation tables and overlays `wiktextract_translation_support` onto the selected upper-bound dataset. The result is `ok` / `wiktextract_support_complete`: `38 / 38` selected senses are supported, `25` senses are newly supported, and all `16 / 16` wave6 families become source-supported. `docs/test_outputs/semantic_non_v10_source_support_conversion_wave6_wiktextract_supported_latest.md` confirms the generic conversion audit now reports `selected_wave_source_supported` with no remaining worklist rows.
- `docs/test_outputs/semantic_wiktextract_non_v10_wave6_upper_bound_latest.md` is the separate Wiktextract example-evidence probe for the same wave. It mines `28` raw example rows and admits `21`, but standalone example coverage completes only `2 / 16` semantic families. This is useful diagnostic evidence but not the main conversion path.
- `docs/test_outputs/semantic_wordnet_def_ex_non_v10_wave6_wiktextract_supported_latest.md` and `docs/test_outputs/semantic_source_admission_cycle_wordnet_def_ex_non_v10_wave6_wiktextract_supported_latest.md` rerun the normal WordNet definition/example evidence path on the now-supported wave6 dataset. The source-admission cycle is `ok` / `analysis_only`: `0` leakage rejects, `11` sense rejects, `61` final admitted rows, and active/shadow semantic contract coverage `16 / 16`. Phrase coverage remains `0 / 16`, so this is source-readiness progress rather than a quality or runtime claim.
- `docs/test_outputs/semantic_source_non_v10_wave6_wiktextract_supported_heldout_margin005_validation_latest.md` and `docs/test_outputs/semantic_source_non_v10_wave6_wiktextract_supported_phrase_margin005_validation_latest.md` add the first fresh independent wave6 held-out suites: `38` active/shadow rows covering all selected active and shadow senses, plus `16` phrase/no-winner rows. The first pass blocked promotion at `2` active/shadow harmful replacements, `1` active false abstain, and `2` phrase/no-winner harmful replacements. The harness now emits configured-case failure traces with active/shadow scores, evidence text, and surface/POS signals so the remaining blockers are inspectable rather than just case IDs.
- The accepted guard repair is deliberately narrow: adding `sold` as a noun-subject predicate cue fixes the `show sold out` false abstain, and an adjective-nominal preemption fixes `upset` as a noun in `an upset in the final`. Temporary wave5 regression checks still pass both active/shadow and phrase/no-winner suites. A broader modifier/noun rescue-threshold change was rejected because it caused a wave5 `dry` false abstain and wave6 active false abstains; the remaining `bear` and `fair` phrase misses are therefore source-coverage gaps, not threshold-policy wins.
- `docs/test_outputs/semantic_wordnet_phrase_control_non_v10_wave6_wiktextract_supported_latest.md` tests the existing WordNet phrase-control miner against the new wave6 phrase suite. It finds one containment row for `in advance`, but not the remaining `bear` animal or `fair` equitable/adjective blockers. `docs/test_outputs/semantic_source_margin_policy_sweep_non_v10_wave6_wiktextract_supported_latest.md` confirms no tested scalar margin clears both wave6 suites, and `docs/test_outputs/semantic_source_failure_class_mining_non_v10_wave6_wiktextract_supported_latest.md` records the current blockers as `leave`, `piece`, `bear`, and `fair`.
...
7. Treat the supported source portfolio plus phrase-control probe as a seed-pass, not a broad proof: both frozen wave5 held-out suites now pass, but the phrase-control probe is same-suite discovery evidence and still needs fresh locked phrase/no-winner validation.
8. Treat the Wiktextract translation-table overlay as the current wave6 source-support conversion path: it closes the `0 / 16` supported-family gap and reruns WordNet source admission to `16 / 16` semantic coverage, but remains analysis-only because the fresh held-out suites expose blocking semantic risk.
9. Fix the wave6 held-out blockers by source/evidence mechanism, not scalar threshold tuning: split `leave` permission-vs-absence with better translation-sense evidence, strengthen board-game `piece` shadow evidence, and add alternate-noun/adjective no-winner source coverage for `bear` animal and equitable `fair`.
10. Decide the runtime phrase-source policy explicitly: either build real phrase-control source adapters for the remaining phrase gaps, add a general pattern policy for phrase/no-winner frames, or accept the already-formalized split where phrase containment remains a separate runtime/publication blocker. The current phrase-control miner proves a useful source-backed path for `7 / 16` wave5 phrase cases, but it is not broad phrase publication proof.
11. Audit runtime feasibility for the sentence-transformer plus surface-POS shape: resource packaging, latency, fallback behavior, and pair/profile gating.
12. Only then revisit broader source generation scale-out.
