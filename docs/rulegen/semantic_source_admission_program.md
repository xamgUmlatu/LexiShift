...
7. Treat the supported source portfolio as materialized but not quality-claimed: the combined portfolio source batch now passes source admission with semantic coverage `16 / 16`, so the next blocker is independent active/shadow and phrase/no-winner held-out rows for those exact `16` families.
