...
- `docs/test_outputs/semantic_non_v10_source_portfolio_wave5_anypos_latest.md` materializes the source portfolio as one variant-aligned source batch instead of trusting the sweep's mixed embedded family payloads. The first attempt exposed a real `rest:reposo` payload/row mismatch; after the materializer rebuilt each selected family from the same supporting variant that supplied its rows, `docs/test_outputs/semantic_source_admission_cycle_non_v10_wave5_source_portfolio_latest.md` passes with `0` leakage rejects, `0` sense rejects, `51` final admitted rows, and semantic contract coverage `16 / 16`. It remains analysis-only because phrase containment is `0 / 16` and independent active/shadow plus phrase held-out cases still need to be added for these selected families.
- `docs/test_outputs/semantic_source_non_v10_wave5_portfolio_heldout_margin005_validation_latest.md` and `docs/test_outputs/semantic_source_non_v10_wave5_portfolio_phrase_margin005_validation_latest.md` add those first independent held-out rows for the materialized portfolio. The first read correctly blocked promotion at `2` harmful replacements / `3` false abstains on the `32`-case active/shadow suite and `3` harmful replacements on the `16`-case phrase/no-winner suite; a no-write margin sensitivity check showed this was not a simple threshold issue. The next syntax candidate exposes local trigger context even when the noun-only phrase guard is disabled, then uses surface syntax only as a narrow near-tie rescue or clear verb-frame preemption. That improves the same frozen suites to `1` harmful / `0` false abstains on active/shadow and `1` harmful / `0` false abstains on phrase/no-winner. It is real progress, not promotion: the remaining blockers are `present:002` (`The present is all we can control`) and `end:001` (`The end table held a small lamp`), and phrase contract coverage is still `0 / 16`.
- `docs/test_outputs/semantic_source_margin_policy_sweep_non_v10_wave5_portfolio_latest.md` reruns the margin-policy question after the syntax candidate. No scalar margin passes both wave5 suites: margins `0` through `0.02` still leave `present:002` and `end:001`, while `0.08+` removes the active harmful replacement only by introducing a `rest:001` false abstain and still leaves `end:001`. This keeps the next step on source/sense/phrase mechanism repair, not threshold tuning.
- `docs/test_outputs/semantic_source_failure_class_mining_non_v10_wave5_portfolio_latest.md` is the current wave5 anti-overfitting ledger. It reports `review` / `fix_blocking_failure_classes`: semantic source admission is complete at `16 / 16` with `51` rows, but held-out harmful replacements remain for `present` and `end`, phrase contract coverage remains absent, and the breadth gap is still `34` more families plus `152` more cases before the default broad-confidence thresholds. Use this report as the next-step source of truth for wave5 blocker cleanup.
...
7. Treat the supported source portfolio as materialized but blocked by held-out validation: the combined portfolio source batch passes source admission with semantic coverage `16 / 16`, but the first independent active/shadow and phrase suites expose harmful replacements. Next work is failure-class repair, not broader promotion or scorer tuning.
