...
Last verified: 2026-04-29 desk audit plus filtered residual source generation, leakage admission, surface-POS prototype guard, non-v10 source-backed held-out validation, failure-class mining, automatic non-v10 inventory candidates, automatic wave construction, no-spend wave-admission sweep, selected-wave held-out validation, wave32/wave64 any-POS source-support sweep, semantic decision-rule comparison plan, forward-only upper-bound probe, source-support conversion audit, loader-only leakage policy, and latest quality-gate artifacts
...
| `2026-04-25` | `Reverse-aux external example-frame batch` | `experiment_next` | `scripts/testing/semantic_reverse_aux_example_frame_batch_en_es.py` now emits a real non-LLM source batch from installed reverse auxiliary sense text; the prompt-queue scope produces `13` rows, covers active reverse-aux text for all `6 / 6` target families and shadow reverse-aux text for `4 / 6`, and has no phrase-control rows | superseded for full-source routing by the all-v10 reverse-aux scope below; keep the queue artifact as the small prompt-slice source floor |
| `2026-04-25` | `Missing-row example-frame generation plan` | `do_now` | `scripts/testing/semantic_llm_example_frame_generation_plan_en_es.py` originally turned the reverse-aux required-family contract gap into a no-spend one-shot plan with `11` requests: `1` active row for `play`, `2` shadow rows for `plant`/`check`, and `8` phrase-control rows; prompt input excluded reviewed case sentences and translation targets | superseded by the multi-candidate planner below; keep the earlier live run as evidence that one-shot missing-row fill was insufficient |
| `2026-04-25` | `Multi-candidate source generation planner` | `do_now` | `scripts/testing/semantic_llm_example_frame_generation_plan_en_es.py` now defaults to active/shadow semantic source generation only, fans missing semantic slots into `20` candidate requests (`10` same-POS `plant` shadow, `5` `check` shadow, `5` `play` active), and keeps phrase-control generation explicit opt-in; the report separates candidate slots, planned raw candidates, semantic candidates, phrase candidates, and same-POS hard strategy counts | next turn should add admission summaries that keep raw generated, structurally accepted, leakage-kept, sense-discriminating, and admitted counts separate before any merge or ablation claim |
| `2026-04-25` | `Source duplicate admission gate` | `do_now` | `scripts/testing/semantic_llm_example_frame_leakage_audit_en_es.py` now rejects duplicate and near-duplicate source rows in addition to benchmark leakage, including optional comparisons against prior source batches via `--prior-batch-json`; regenerated source-coverage audits show `3` benchmark leakage hits, `0` duplicate hits, and `19` kept rows, while manual leakage-safe replacement batches remain `ok` with `0` duplicate hits | next turn should add the sense-discrimination gate before merge so multi-candidate rows must prove active/shadow separation, not merely pass leakage and duplication filters |
| `2026-04-25` | `Sense-discrimination admission gate` | `do_now` | `scripts/testing/semantic_llm_example_frame_sense_discrimination_audit_en_es.py` now audits active/shadow rows before merge by comparing each row's evidence text to its intended dataset sense and competing senses; cheap lexical/TF-IDF screening admits only `31 / 52` semantic rows on the safe-v2 composite, while sentence-transformer screening with `min_intended_score=0.5` and `min_margin=0.0` admits `50 / 52`, rejects only two reverse-aux rows, and preserves the full-`v10` downstream best read at `100.0%` accuracy / `100.0%` recall / `0` harmful / `0` false abstains | next turn should wrap generation/normalization/leakage/sense-admission/merge/contract/ablation into one source-admission cycle command so future source batches cannot skip a gate |
| `2026-04-25` | `Source-admission cycle wrapper` | `do_now` | `scripts/testing/semantic_source_admission_cycle_en_es.py` now runs the no-spend admission cycle end to end for an existing candidate batch: candidate leakage/duplicate audit, duplicate comparison against the base batch, merge, final-composite sense admission, split contract, ablation, and residual extraction. The latest cycle artifact remains `review` / `analysis_only`: `16` duplicate rows rejected, `2` final sense rows rejected, final admitted composite `55` rows, semantic coverage `19 / 19`, phrase coverage `8 / 19`, and best full-`v10` ablation still `100.0%` / `100.0%` / `0` / `0` on the sentence-transformer surface-POS lane | next turn should run this wrapper on a fresh multi-candidate batch or external-source adapter output instead of continuing to rely on the hand-composed safe-v2 composite; runtime publication remains blocked until fresh source admission and the phrase/source policy are settled |
| `2026-04-25` | `All-v10 reverse-aux adapter admission` | `experiment_next` | `scripts/testing/semantic_reverse_aux_example_frame_batch_en_es.py --scope all_dataset_families` now extracts a broader real-source adapter batch from installed reverse aux text: `35` rows across all `19` v10 families, active aux coverage `18 / 19`, shadow aux coverage `17 / 19`, and no benchmark leakage. The source-admission cycle with `--empty-base` rejects `3` rows at sense admission, leaves semantic contract coverage at `14 / 19` and phrase coverage at `0 / 19`, and the best full-`v10` candidate row reaches only `90.5%` accuracy / `86.8%` recall / `4` harmful / `5` false abstains | use this as the fresh non-manual source floor and gap map, not as a promotion candidate; next source work should target true examples/retrieval for `cell`, `check`, `play`, `plant`, and `report`, plus phrase-lane coverage, then rerun the same cycle |
...
| `2026-04-25` | `Source-coverage ablation and leakage-safe replacement` | `experiment_next` | the new ablation harness shows source coverage and scorer choice are both first-order: the unfiltered active/shadow coverage run accepted `22 / 22` rows but leakage admission filtered `3`, and the failed plant remediation filtered `1 / 1`; after merging the filtered coverage batch with `5` leakage-safe internal replacement rows, the full-`v10` prototype read reaches `100.0%` accuracy / `100.0%` recall / `0` harmful / `0` false abstains only on the sentence-transformer `active_shadow_containment_surface_pos` lane, while lexical/TF-IDF lanes still trail (`8` false abstains on the full all-family candidate row) | keep this as analysis-only: the source contract is still `review` because `11` non-queue families lack phrase-control rows, but the phrase ablation shows `generated_no_phrase` also reaches `100.0%` on the sentence-transformer lane, so next work should harden the leakage-safe source-admission/generation path and scorer dependency before considering runtime publication |
| `2026-04-25` | `WordNet external source adapter admission` | `experiment_next` | `scripts/testing/semantic_wordnet_example_frame_batch_en_es.py` now builds a local WordNet example-frame source from installed English WordNet JSON, preferring entry sentence frames over generic synset examples when available; by itself the WordNet batch is not promotion-ready, but after merging with the all-`v10` reverse-aux floor it closes the semantic contract to `19 / 19`, improves the best full-`v10` read to `99.0%` accuracy / `97.4%` recall / `0` harmful / `1` false abstain, and narrows the only residual semantic miss to `plant:001` | keep WordNet as a broad no-spend external source lane and as evidence that real examples beat another prompt-only loop; phrase coverage remains `0 / 19`, and the next source step should target the final same-POS living-plant gap from a corpus rather than a generic generated plant prompt |
| `2026-04-25` | `Raw Wiktextract residual adapter admission` | `do_now` | `scripts/testing/semantic_wiktextract_example_frame_batch_en_es.py` now reads the local raw Wiktextract gzip directly for residual examples; the plant run at `--min-link-score 0.08` produced one living-plant active row and two industrial-plant shadow rows, and the reverse-aux plus WordNet plus Wiktextract source-admission cycle reports `ok` / `promotion_candidate` with `67` admitted rows, `0` leakage rejects, `0` sense rejects, semantic coverage `19 / 19`, and a best full-`v10` ablation of `100.0%` accuracy / `100.0%` recall / `0` harmful / `0` false abstains | treat this as the first promotion-candidate offline active/shadow semantic source lane, not runtime behavior; next work is to formalize the split promotion policy, then run held-out/non-benchmark validation and runtime-packaging feasibility before any source publication |
| `2026-04-25` | `Split semantic-source promotion policy` | `do_now` | `scripts/testing/semantic_source_admission_cycle_en_es.py` now exposes the policy split directly in source-cycle JSON and Markdown: the offline promotion lane is `semantic_active_shadow`, phrase containment has its own `review` lane when missing, and the legacy combined contract no longer hides whether semantic admission itself passed | next work should stop treating phrase-control source coverage as a blocker to offline semantic-source promotion, but keep it as an explicit runtime/publication policy blocker until a real phrase source adapter or accepted no-phrase runtime policy exists |
| `2026-04-25` | `en-es expansion opportunity roadmap` | `do_now` | `docs/rulegen/semantic_en_es_expansion_opportunity_roadmap.md` now preserves the full post-promotion-candidate idea set: reference-lane freezing, held-out validation, source-adapter expansion, admission hardening, algorithm variants, phrase policy, runtime capture, no-spend sweeps, test expansion, governance, and more fundamental product options | use the roadmap as the catch-all backlog for this new world of possibilities; next execution should still start with held-out/non-benchmark validation before broad source scaling or runtime publication |
| `2026-04-25` | `Held-out source validation and active-related WordNet repair` | `do_now` | `scripts/testing/semantic_source_heldout_validation_en_es.py` adds a seed non-benchmark active/shadow validation slice over `6` families and `12` cases; the first run caught `3` false abstains, the narrowed surface-POS predicate-frame fix reduced that to `1`, and active-side WordNet direct-hyponym evidence for living-plant rows produces a clean source-admission lane at `79` admitted rows, full-`v10` `100.0%` accuracy / `100.0%` recall / `0` harmful / `0` false abstains, and held-out `100.0%` / `100.0%` / `0` / `0` | superseded by the v2 all-family held-out lane below; keep this row as the seed-history explanation |
| `2026-04-25` | `Semantic source reference-lane manifest` | `do_now` | `docs/test_inputs/semantic_routing/semantic_source_reference_lane_en_es_v1.json` and `scripts/testing/semantic_source_reference_lane_en_es.py` initially froze the clean seed offline lane across source-cycle status, held-out validation, admitted evidence identity, row counts, relation counts, active-related WordNet repair coverage, and the embedded held-out policy blocker split | superseded by the v2 reference-lane refresh below; continue using the manifest as the single frozen-lane pointer |
| `2026-04-25` | `Held-out-aware source-cycle policy report` | `do_now` | `scripts/testing/semantic_source_admission_cycle_en_es.py` now accepts `--heldout-validation-json`; a failing provided held-out artifact blocks offline promotion status, while a passing held-out report changes the runtime blocker from `held_out_non_benchmark_validation` to `broader_heldout_breadth` | next work should grow the held-out corpus deliberately rather than rereading the current held-out slice as runtime-publication confidence |
| `2026-04-25` | `Held-out v2 all-family breadth` | `do_now` | `docs/test_inputs/semantic_routing_cases/en_es_source_heldout_cases_v2.json` expands the active/shadow held-out suite to all `19` v10 families with `38` cases. The first v2 run exposed two concrete misses: `play:001` was wrongly preempted because surface-POS did not recognize irregular predicate `won`, and `cell:001` lacked active biology depth because WordNet related evidence only traversed one hyponym hop. The repaired lane in `docs/test_outputs/semantic_source_heldout_validation_v2_latest.md` now passes at `100.0%` accuracy / `100.0%` recall / `0` harmful / `0` false abstains | next held-out work should add phrase-sensitive rows and non-v10 families; do not tune further on the v2 active/shadow cases |
| `2026-04-25` | `WordNet cell-depth and admitted-delta source lane` | `do_now` | `scripts/testing/semantic_wordnet_example_frame_batch_en_es.py` now supports bounded `--related-hyponym-depth`, and `scripts/testing/semantic_source_admission_cycle_en_es.py` can emit a candidate-admitted delta batch so raw broad source harvests do not become frozen evidence by accident. The raw depth-3 cell harvest produced `86` rows; admission rejected `29` duplicate rows and `3` sense-weak rows, then froze only the `54` admitted delta rows. The final source lane `docs/test_outputs/semantic_source_admission_cycle_wordnet_active_related_plant_cell_depth3_heldout_v2_policy_latest.md` is `ok` / `promotion_candidate` with `133` admitted rows, `0` leakage rejects, `0` sense rejects, semantic coverage `19 / 19`, v2 held-out pass, and full-`v10` `100.0%` / `100.0%` / `0` / `0` | keep this as the current offline semantic-source reference; runtime remains blocked on phrase-source policy, broader held-out breadth, and sentence-transformer/evidence packaging feasibility |
| `2026-04-25` | `Reference lane refreshed to v2` | `do_now` | `docs/test_inputs/semantic_routing/semantic_source_reference_lane_en_es_v1.json` points at the v2 source-cycle, v2 held-out artifact, and the `133`-row admitted evidence batch. `scripts/testing/semantic_source_reference_lane_en_es.py` also checks that the frozen evidence contains active related WordNet depth for the `cell` repair, and the initial v2 reference report froze `46` checks with `0` failures | next work should preserve this reference while adding phrase-heldout and non-v10 held-out rows; broad source scaling should compare against this fixed lane |
| `2026-04-26` | `Phrase/no-winner held-out boundary` | `experiment_next` | `docs/test_inputs/semantic_routing_cases/en_es_source_phrase_heldout_cases_v1.json` adds one non-benchmark phrase/no-winner abstain case for each of the `19` v10 families. The first run exposed one harmful replacement, `match:001` (`Those numbers match the totals on the receipt.`), a verb-frame no-winner case where neither active/shadow noun evidence nor existing phrase containment had authority | superseded by the subject-trigger-object phrase guard below; keep this as evidence that phrase policy must be measured separately from active/shadow semantic promotion |
| `2026-04-26` | `Subject-trigger-object phrase guard` | `do_now` | `core/lexishift_core/rulegen/semantic_routing_runtime_scoring.py` now treats all-noun-family frames like `numbers match the` as phrase/no-winner preemption instead of relying on active/shadow semantic evidence. `docs/test_outputs/semantic_source_phrase_heldout_validation_latest.md` now passes at `100.0%` accuracy / `0` harmful / `0` false abstains over `19` phrase cases, and `docs/test_outputs/semantic_source_heldout_validation_v2_latest.md` still passes at `100.0%` / `100.0%` / `0` / `0` over `38` active/shadow cases | keep runtime phrase-source policy as a blocker until this is broadened beyond one phrase case per family and paired with explicit phrase-source/pattern provenance |
| `2026-04-26` | `Phrase v2 calibrated-margin candidate` | `do_now` | `docs/test_inputs/semantic_routing_cases/en_es_source_phrase_heldout_cases_v2.json` doubles the phrase/no-winner slice to `38` cases. The zero-margin run found one harmful replacement, `board:002` (`The new designer came on board last month.`), where active beat shadow by only `0.0014`; a general `min_margin=0.005` candidate artifact passes phrase v2, active/shadow v2, and the full-`v10` source ablation with `0` harmful and `0` false abstains. The reference lane now tracks this as a separate phrase-policy candidate instead of changing the frozen offline active/shadow lane. | keep this as bounded near-tie evidence only; the independent phrase challenge below supersedes margin-only promotion as the next hypothesis |
| `2026-04-26` | `Margin policy sweep harness` | `do_now` | `scripts/testing/semantic_source_margin_policy_sweep_en_es.py` now sweeps candidate margins across active/shadow v2, phrase v1, phrase v2, phrase challenge v1, phrase stress v1, and full-`v10` source ablation in one report. After the phrase-pattern repair and stress suite, `docs/test_outputs/semantic_source_margin_policy_sweep_latest.md` is `ok` / `margin_candidate_found`: `0.005` is the smallest passing margin, `0.01` also passes, `0`/`0.001` retain the phrase v2 `board:002` harmful replacement, and `0.02+` introduces active false abstains in held-out and full-`v10` rows. | next work should stress this current-suite candidate on non-v10 held-out rows before promoting any runtime margin; optimize scorer reuse later if runtime becomes a bottleneck |
| `2026-04-26` | `Independent phrase challenge v1` | `experiment_next` | `docs/test_inputs/semantic_routing_cases/en_es_source_phrase_challenge_cases_v1.json` adds a fresh `19`-case no-winner phrase challenge across the v10 families. Its first `min_margin=0.005` run exposed `2` harmful replacements and `0` false abstains: `ball:001` (`keep the ball rolling`) and `file:001` (`Customers file past...`). These were not near-tie misses, so margin-only policy was not the right hypothesis. | superseded by the phrase-pattern repair below; keep this as evidence that phrase policy needs explicit pattern/source provenance rather than scalar margins alone |
| `2026-04-26` | `Phrase-pattern repair for challenge v1` | `do_now` | `core/lexishift_core/rulegen/semantic_routing_runtime_scoring.py` now adds two guarded phrase-control signals: a subject-trigger-particle frame for high-confidence particles such as `file past`, and a conservative `get/keep/set ... the ball rolling` idiom frame. The guard explicitly avoids literal active frames such as `Each cell in the tissue...` and `She saw the ball rolling down the hill.` `docs/test_outputs/semantic_source_phrase_challenge_v1_margin005_validation_latest.md` now passes at `100.0%` decision accuracy with `0` harmful replacements and `0` false abstains. | superseded by the fresh phrase stress suite below; keep this as the implementation record for the current pattern candidate |
| `2026-04-26` | `Fresh phrase stress v1` | `do_now` | `docs/test_inputs/semantic_routing_cases/en_es_source_phrase_stress_cases_v1.json` adds a second fresh `19`-case no-winner phrase suite after the phrase-pattern repair, covering new idiom, compound, phrasal-verb, and cross-POS verb-frame rows. `docs/test_outputs/semantic_source_phrase_stress_v1_margin005_validation_latest.md` passes at `100.0%` decision accuracy with `0` harmful replacements and `0` false abstains, and the default margin sweep remains clean with `6` suites and `36` rows. | next work should move outside the current v10 family set by adding non-v10 ambiguous families and source/phrase stress rows |
| `2026-04-26` | `Non-v10 phrase signal audit` | `do_now` | `scripts/testing/semantic_phrase_policy_signal_audit_en_es.py` audits phrase-control signal firing without source evidence or semantic scoring, so non-v10 phrase rows can be screened before source coverage exists. `docs/test_outputs/semantic_phrase_policy_signal_non_v10_latest.md` passes `16 / 16` rows across `8` non-v10 ambiguous heads with `0` false positives and `0` false negatives. | next work should add source-backed non-v10 active/shadow plus phrase rows so these signal-only cases can graduate into end-to-end held-out validation |
| `2026-04-26` | `Non-v10 source-backed WordNet definition probe` | `do_now` | `docs/test_inputs/semantic_routing_cases/en_es_source_non_v10_probe_v1.json` adds `8` non-v10 ambiguous active/shadow families and `docs/test_inputs/semantic_routing_cases/en_es_source_non_v10_heldout_cases_v1.json` adds `16` held-out cases. Example-preferred WordNet found coverage but lost `4` active rows at sense admission; definition-preferred WordNet admits `18 / 18` rows with `0` leakage rejects and `0` sense rejects. `docs/test_outputs/semantic_source_non_v10_heldout_v1_margin005_validation_latest.md` now passes at `100.0%` accuracy / `100.0%` recall / `0` harmful / `0` false abstains after tightening surface-POS active rescue so mixed noun/verb shadow families do not over-rescue alternate-noun shadows like `violin case`. | treat this as strong seed evidence for the source-backed non-v10 expansion path, not broad en-es proof; next work should add another non-v10 wave and phrase-sensitive rows without tuning on this held-out slice |
| `2026-04-28` | `Non-v10 source failure-class mining` | `do_now` | `scripts/testing/semantic_source_failure_class_mining_en_es.py` now reads the current non-v10 definition-preferred admission, clean held-out validation, example-preferred comparator, source extraction reports, and margin sweep as one no-spend anti-handcrafting report. `docs/test_outputs/semantic_source_failure_class_mining_latest.md` is `review` / `seed_pass_expand_inventory`: there are `0` blocking semantic-promotion classes and the held-out slice is clean, but medium manual-overfit risk remains because the proof is still `8` families / `16` cases, seed ablation has `4` false abstains (`rock`, `point`, `date`), phrase contract coverage is absent, and broad-confidence thresholds still need `42` more families / `184` more cases. | next work should build automatic non-v10 inventory candidate generation and rerun source extraction, admission, held-out validation, margin sweep, and failure-class mining on a larger wave before making another algorithmic tweak |
| `2026-04-28` | `Automatic non-v10 inventory candidates` | `do_now` | `scripts/testing/semantic_non_v10_inventory_candidates_en_es.py` builds a no-spend WordNet-backed candidate inventory after excluding current v10 and seed non-v10 triggers. `docs/test_outputs/semantic_non_v10_inventory_candidates_latest.md` emits `75` ranked ambiguous English headwords with POS/sense/source-availability metadata, but it deliberately does not pretend those are admitted en-es families because active/shadow Spanish target construction is still missing. | next work should select a bounded wave from this queue, construct the missing Spanish active/shadow target families, then rerun WordNet definition-preferred extraction, admission, held-out validation, margin sweep, and failure-class mining |
| `2026-04-28` | `Automatic non-v10 wave construction and sweep` | `do_now` | `scripts/testing/semantic_non_v10_wave_builder_en_es.py` now builds draft Spanish active/shadow families from the inventory using local Wiktionary, reverse Wiktionary, FreeDict, and WordNet link support, while rejecting same-visible-target active/shadow pairs to preserve the binary UX. `scripts/testing/semantic_non_v10_wave_admission_sweep_en_es.py` runs the no-spend constructor/extraction/admission grid. The fixed-eight control reaches `7 / 8` semantic-complete families, while `docs/test_outputs/semantic_non_v10_wave_admission_sweep_wave16_latest.md` proves the better pool-first design: `--wave-size 16 --selection-size 8` finds `9` semantic-complete families and materializes an `8 / 8` admission-selected draft wave. `docs/test_outputs/semantic_source_admission_cycle_wordnet_def_ex_non_v10_wave2_selected_latest.md` confirms the selected wave has `30` admitted rows, `0` leakage rejects, and semantic coverage `8 / 8`. | superseded by selected-wave held-out validation below; keep the important design lesson as pool-first admission selection rather than fixed-eight tuning |
| `2026-04-28` | `Admission-selected non-v10 wave held-out validation` | `do_now` | `docs/test_inputs/semantic_routing_cases/en_es_source_non_v10_wave2_selected_heldout_cases_v1.json` adds `16` active/shadow held-out rows and `docs/test_inputs/semantic_routing_cases/en_es_source_non_v10_wave2_selected_phrase_cases_v1.json` adds `8` phrase/no-winner rows for the admission-selected automatic wave. The first run exposed two surface-POS false preemptions (`safe use of...`, `days of rest`) and one phrase harmful replacement (`The rest of the team...`); `scripts/testing/semantic_llm_surface_pos_support.py` now recognizes conservative `of` noun-complement frames, and `core/lexishift_core/rulegen/semantic_routing_runtime_scoring.py` adds a guarded `the rest of` phrase-control frame. Both selected-wave held-out suites now pass with `0` harmful replacements and `0` false abstains, `docs/test_outputs/semantic_source_non_v10_wave2_selected_margin_policy_sweep_latest.md` passes all tested margins from `0` through `0.05`, and `docs/test_outputs/semantic_source_failure_class_mining_non_v10_wave2_selected_latest.md` reports `0` blocking semantic-promotion classes over `24` held-out cases across `8` families. | next work should scale the same pool-first construction to the next automatic non-v10 inventory tier; do not tune this selected wave further unless the broader run reproduces a failure class |
| `2026-04-28` | `Wave32/wave64 any-POS and source-support upper bound` | `do_now` | `scripts/testing/semantic_non_v10_wave_builder_support.py` now extracts the constructor's POS-pair selection helpers so the builder stays below the health limit while supporting both `noun_verb` and `any_cross_pos` family shapes. `scripts/testing/semantic_wordnet_source_adapter_support.py` now records WordNet sense rank and uses that source prior for weak lexical links, while `scripts/testing/semantic_non_v10_wave_admission_sweep_en_es.py` can keep conservative construction thresholds separate from broader extraction thresholds. `docs/test_outputs/semantic_non_v10_wave_admission_sweep_wave64_anypos_latest.md` confirms the supported constructor is still exhausted at the same `16` families even when asked for `64`, but the latest source portfolio is `ok` / `semantic_complete_source_portfolio_found`: best single variant `15 / 16`, portfolio `17` semantic-complete supported families, and a materialized `16`-family portfolio-selected dataset/queue tagged `reverse_or_freedict_required`. `docs/test_outputs/semantic_non_v10_source_support_conversion_wave4_anypos_supported_probe_latest.md` verifies those `16` supported families have `36 / 36` translation-supported senses. `docs/test_outputs/semantic_non_v10_wave_admission_sweep_wave32_anypos_unsupported_latest.md` relaxes reverse/FreeDict support as a deliberate upper-bound probe and reaches `31 / 32` semantic-complete families plus a materialized `16`-family `forward_only_upper_bound` control dataset/queue, but `docs/test_outputs/semantic_non_v10_source_support_conversion_wave3_anypos_latest.md` shows only `3 / 16` selected upper-bound families are fully source-supported. | next work should materialize the supported source-portfolio evidence as one admitted batch, add held-out active/shadow plus phrase cases for the selected `16`, and separately convert upper-bound selected families into reverse-supported or reviewed source-backed rows before any quality claim |
| `2026-04-29` | `Semantic decision-rule comparison plan` | `do_now` | `docs/rulegen/semantic_decision_rule_comparison_plan.md` now formalizes the open question behind `similarity(context, active) - max_similarity(context, shadow)`: the current rule is a one-versus-nearest-competitor prototype classifier and should remain the control, not an assumed optimum. The plan decomposes the hidden `active_score = similarity(c, a)` expression into context representation, sense/source representation, scoring function, row aggregation, final decision rule, phrase/no-winner handling, and source-data polysemy checks. It also preserves the alternative algorithms to test: ratio, softmax, pairwise active-beats-shadows, abstention-first agreement, source-weighted top-k rows, cross-encoder, entailment, calibrated classifier, learned reranker, family/cluster thresholds, and required negative controls. | next implementation should create the first no-spend comparison matrix with frozen source evidence and held-out suites, keeping the current margin rule as the baseline and recording harmful replacements, false abstains, phrase failures, and family-level regressions separately |
