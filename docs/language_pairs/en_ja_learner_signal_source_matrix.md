...
| Aozora Bunko morphology aggregate (`aozora-word`) | `research-local` | Local sidecar for learner-difficulty experiments; not consumed by the accepted scorer. | Aozora token rank/pmw, surface/base/reading/POS counts, work dispersion, author dispersion, orthography/work metadata, inferred work accessibility bands, modern-vs-old exposure counts, NDC children/youth exposure counts, accessible/hard work exposure counts, token-level lexical-context features such as accessible-work exposure, hard-work exposure, old-literary risk context, and modern-child-accessible context | Built from `青空文庫形態素解析データ集` full UTF-8 morphological CSV into `$DATA_ROOT/frequency_packs/freq-ja-aozora-word/main.sqlite`. `scripts/testing/build_aozora_lexical_context_features_ja.py` turns the existing `token_context_profile` and `work_profile` tables into a reusable sidecar feature SQLite with `work_audience_axis_score`, `token_audience_summary`, and `token_audience_feature`. Best treated as literary/book exposure evidence and upper-tail tiebreaking, not a replacement for BCCWJ/Tubelex. Source license is CC BY-SA 2.1 JP, so product redistribution policy needs explicit review before shipping derived artifacts. |
| Aozora work-audience metadata probe | `research-local` | Stress-test sidecar for selected Aozora works; not consumed by the accepted scorer. | Local Aozora work-profile stats, Aozora card labeled fields, `初出`/`底本`/`備考` juvenile or warning terms, download-link candidates, NDL Search OpenSearch title/creator results, NDL publisher/date/series/subject/transcription cues, NDL SRU record count/facets/NDC/year/repository distributions, Yozora juvenile-card match and category size, optional Bungo Search for Kids listing match, Bungo direct detail read-time/PV/excerpt/base-book cues, opportunistic Wikipedia/Wikidata category/description cues, plus normalized SQLite feature rows such as positive-source count, NDL audience-item count, SRU K-NDC facet count, Bungo listing count, Bungo character/PV cues, and local work accessibility percentiles | Implemented as `scripts/testing/fetch_aozora_work_audience_metadata_ja.py`. It is deliberately work-id scoped, cached, rate-limited, and guarded against broad runs. `--output-json` preserves the full probe artifact; optional `--output-sqlite` writes run-scoped tables for works, provider payloads, fetch provenance, Aozora card fields/downloads, NDL OpenSearch/SRU rows, Yozora matches, Bungo detail/listing rows, Wikipedia/Wikidata rows, and derived `work_audience_feature` records. Interpretation rubric: `docs/srs/srs_learner_difficulty_aozora_audience_signal_rubric_en_ja.md`. Bungo direct detail pages are not treated as audience proof unless the work is also found in scanned for-Kids listing pages. NDL metadata may be public API data, but provider terms, credit, and product-use requirements still need review before shipping derived product artifacts. |
