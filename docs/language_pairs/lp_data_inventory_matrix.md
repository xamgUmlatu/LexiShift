...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| English target (`en-en`, `de-en`, `es-en`) and English source-frequency prior (`en-de`) | Frequency DB (SQLite) | `freq-en-leipzig-default`; legacy/manual fallback `freq-en-coca` | `https://downloads.wortschatz-leipzig.de/corpora/eng_news_2025_1M.tar.gz`; fallback `https://www.wordfrequency.info/samples/lemmas_60k.txt` | `$DATA_ROOT/frequency_packs/freq-en-leipzig-default/main.sqlite`; fallback `$DATA_ROOT/frequency_packs/freq-en-coca.sqlite` | Leipzig downloadable corpus expected CC BY; fallback COCA sample remains manual/review-required | Required table: `frequency`. Required by code: case-insensitive `lemma` + one rank column (`core_rank`/`rank`/`id`/`index`) + one frequency column (`pmw`/`frequency`/`freq`/etc). | Leipzig English pack is frequency-only today (`pos` empty); fallback COCA has compact `pos` tags when manually supplied. | managed pack id `freq-en-leipzig-default`; fallback keys `freq-en-coca` / `freq-en-coca.sqlite` | Active default for EN-target SRS LPs and `en-de` source-frequency rulegen prior once installed |
...
| Spanish target (`en-es`, `es-es`) | Frequency DB (SQLite) | `freq-es-spalex-v1` | `https://ndownloader.figshare.com/files/11826623` | `$DATA_ROOT/frequency_packs/freq-es-spalex-v1/main.sqlite` or legacy-flat `$DATA_ROOT/frequency_packs/freq-es-spalex-v1.sqlite` | CC BY 4.0, verified from upstream; generated artifacts must preserve attribution/provenance | Required table: `frequency` with same minimum column contract above. Current SPALEX pipeline writes `lemma`, rank/frequency-style prevalence fields, `source_family`, and metadata sidecars. | SPALEX itself is frequency-only; use `pos-es-ud-ancora-v1` as the current product POS overlay for ES admission parity. | managed pack id `freq-es-spalex-v1`; old `freq-es-cde` settings are retired and ignored by runtime fallback | Active default for ES-target SRS LPs |
...
- Current Spanish publication caveat:
  - `freq-es-spalex-v1` is the active admission/default frequency source.
  - `freq-es-cde` is retained only as a historical/manual benchmark and is no longer an automatic fallback.
  - `wiktionary-es-en` is the current primary `en-es` rulegen/publication source; `freedict-es-en` remains fallback coverage.
