...
- compiled-resource `en-es` runs now also project config-driven candidate score rows into the runtime path:
  ...
  - compact selected-row-table preparation now also reuses equivalent selected-row results across distinct configs by compiled row-selection signatures, so the cache now keys on accepted row groups, target-ranked row order, reverse-hygiene signals, and threshold pass/fail rows instead of score-table object identity or raw confidence payloads
