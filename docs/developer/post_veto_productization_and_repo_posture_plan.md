...
- `en-es-active-only-combined-full-v1-tranche-005` is the latest automated-clean generated-data pack.
- The pack has `261` source-target families, `534` normalized evidence rows, and installs as `261` helper rules.
...
- preserve the tranche-005 pack artifacts and install path,
...
- coverage increases from the current `261 / 570` automated-clean family checkpoint with no hidden data mutation,
