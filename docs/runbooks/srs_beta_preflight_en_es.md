...
| Data-pack readiness | en-es frequency/dictionary/topic resources resolve. If frequency/dictionary data is missing, the guided SRS setup modal names learner-facing resource categories, opens LexiShift GUI Resource settings through the helper with the en-es install card focused, reuses the existing GUI instance when it is already open, and retrying after install uses the same setup flow. |
