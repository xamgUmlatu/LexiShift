...
Scope:
- Current Chrome extension popup behavior (`apps/chrome-extension/content/ui.js`).
- Core examples:
...
Current behavior:
- Implemented in `buildJapaneseScriptModule(target)` in `apps/chrome-extension/content/ui.js`.
...
Current behavior:
- Implemented as feedback bar in `ensureFeedbackPopup()` in `apps/chrome-extension/content/ui.js`.
...
- `debugLog(...)` in `apps/chrome-extension/content/ui.js`
