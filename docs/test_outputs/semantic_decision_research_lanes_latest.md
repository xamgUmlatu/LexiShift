...
- Status: `ok`
- Generated: `2026-04-28T23:36:54Z`
...
| non_v10_breadth_validation | active_source_program | heldout_breadth | analysis_only | Automatic non-v10 source-supported waves are promising but still need broader held-out proof and phrase-contract coverage before runtime-quality claims. The wave64 any-POS supported sweep found a 16-family semantic source portfolio, and the materialized portfolio passes source admission with 51 final admitted rows, 0 leakage rejects, 0 sense rejects, and semantic contract coverage 16 / 16. A syntax candidate that exposes local trigger context outside noun-only phrase guard and limits modifier rescue to near ties improves the frozen wave5 held-out suites: active/shadow is now 1 harmful / 0 false abstains across 32 cases, and phrase/no-winner is now 1 harmful across 16 cases. A current wave5 margin sweep finds no scalar margin that passes both suites, so the remaining blockers are present-as-time-noun and end-table compound/no-winner handling rather than threshold selection. Phrase source coverage remains 0 / 16. | Do not promote yet. Fix the two remaining reusable blocker classes without tuning thresholds: review the present family/sense boundary and add or generate source-backed compound/phrase no-winner evidence for end-table-style cases. Then rerun admission, both held-out validations, failure-class mining, and a margin/policy sweep before any breadth expansion claim. |
