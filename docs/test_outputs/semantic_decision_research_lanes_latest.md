...
- Status: `ok`
- Generated: `2026-04-28T22:33:21Z`
...
| non_v10_breadth_validation | active_source_program | heldout_breadth | analysis_only | Automatic non-v10 source-supported waves are promising but still need broader held-out proof and phrase-contract coverage before runtime-quality claims. The wave64 any-POS supported sweep found a 16-family semantic source portfolio, and the materialized portfolio now passes the source-admission cycle with 51 final admitted rows, 0 leakage rejects, 0 sense rejects, and semantic contract coverage 16 / 16. It remains analysis-only because phrase containment is 0 / 16 and no independent held-out cases exist for these selected families yet. | Add independent active/shadow and phrase/no-winner held-out cases for the materialized 16-family portfolio, then validate against the exact admitted source batch before any promotion or scorer comparison claim. |
