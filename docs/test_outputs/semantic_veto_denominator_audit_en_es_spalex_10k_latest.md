...
- Status: `review`
- Decision: `semantic_veto_denominator_audit_needs_review`
- Generated: `2026-05-16T18:45:22+00:00`
- SRS learner-target universe: `10000` target lemmas from `10000` seed rows
- Candidate bridge source-target families: `4260`
- Semantic-veto replacement denominator: `570` source-target families
- Denominator scope: `current_active_only_plan_not_expanded_candidate`
...
When the SRS Zipf bridge source-target count differs from the active-only plan denominator, this audit is a current-plan accounting check, not an expanded-candidate coverage report.

The bridge input has `4260` source-target families, while the active-only plan denominator has `570` (`delta=3690`).

## Source Pipeline

| Step | Current Value |
| --- | --- |
| Frequency DB | `/tmp/lexishift-spalex-audit/data-root/frequency_packs/freq-es-spalex-expanded-v1/main.sqlite` |
| Seed top N requested | `10000` |
| Seed rows | `10000` |
| Unique SRS target lemmas | `10000` |
| Rulegen targets | `10000` |
| Rulegen rules | `4260` |
| Source-target pairs | `4260` |
...
| Check | Result |
| --- | --- |
| `bridge_and_plan_denominator_match` | `False` |
