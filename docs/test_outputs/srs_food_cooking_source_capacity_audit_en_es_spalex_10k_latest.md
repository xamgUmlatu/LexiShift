# en-es Food/Cooking Source Capacity Audit

- Status: `ok`
- Decision: `food_cooking_source_capacity_audit_completed`
- Generated: `2026-05-19T02:52:43+00:00`
- Full local Kaikki food-signal lemmas: `2122`
- Current frequency frontier food-signal lemmas: `265`
- Outside current frontier: `1857`

## Findings

- `PASS` `full_source_food_signals_present`: Local Kaikki has food/cooking signal supply.

## Source Capacity

- Tier counts: `{'A': 80, 'B': 76, 'C': 1209, 'D': 757}`
- Confidence bands: `{'high': 152, 'inventory': 179, 'medium': 1142, 'review': 649}`

### Top Source Labels

- `food_gloss_pattern`: 610
- `foods`: 264
- `food_translation_pattern`: 147
- `fruits`: 134
- `fish`: 110
- `cooking`: 101
- `vegetables`: 86
- `meats`: 72
- `sauces`: 72
- `legumes`: 69
- `beverages`: 51
- `spices_and_herbs`: 42
- `desserts`: 42
- `seafood`: 41
- `alcoholic_beverages`: 33
- `herbs`: 31
- `breads`: 29
- `soups`: 26
- `grains`: 21
- `cheeses`: 18

## Common Food Probe Coverage

| Lemma | In Current Frontier | Policy Signal | Best Signal |
| --- | --- | --- | --- |
| `comida` | `True` | `True` | `B:high:primary_translation:food` |
| `comer` | `True` | `False` | `` |
| `cocinar` | `True` | `True` | `C:high:cooking` |
| `cocina` | `True` | `True` | `C:high:cooking` |
| `restaurante` | `True` | `False` | `` |
| `agua` | `True` | `True` | `C:medium:beverages` |
| `vino` | `True` | `True` | `C:medium:alcoholic_beverages` |
| `pan` | `True` | `True` | `B:high:primary_translation:bread` |
| `arroz` | `True` | `True` | `C:review:foods` |
| `pollo` | `True` | `True` | `B:high:primary_translation:chicken` |
| `carne` | `True` | `True` | `C:medium:meats` |
| `huevo` | `True` | `True` | `B:high:primary_translation:egg` |
| `leche` | `True` | `True` | `A:high:food` |
| `queso` | `True` | `True` | `B:high:primary_translation:cheese` |
| `tomate` | `True` | `True` | `B:high:primary_translation:tomato` |
| `patata` | `True` | `True` | `B:high:primary_translation:potato` |
| `papa` | `True` | `True` | `C:medium:vegetables` |
| `azúcar` | `True` | `True` | `B:high:primary_translation:sugar` |
| `sal` | `True` | `True` | `B:high:primary_translation:salt` |
| `sopa` | `True` | `True` | `C:medium:soups` |
| `fruta` | `True` | `True` | `B:high:primary_translation:fruit` |
| `verdura` | `True` | `True` | `B:high:primary_translation:vegetable` |
| `pescado` | `True` | `True` | `C:medium:seafood` |
| `cerveza` | `True` | `True` | `B:high:primary_translation:beer` |
| `café` | `True` | `False` | `` |
| `té` | `True` | `True` | `B:high:primary_translation:tea` |

## Outside-Frontier Examples

- `a fuego lento`: `A` `high` via `cooking`
- `a la romana`: `A` `high` via `cooking`
- `achara`: `A` `high` via `cooking`
- `ahumado`: `A` `high` via `cooking`
- `al gusto`: `A` `high` via `cooking`
- `al vapor`: `A` `high` via `cooking`
- `amasado`: `A` `high` via `cooking`
- `brasear`: `A` `high` via `cooking`
- `cebiche`: `A` `high` via `cooking`
- `criadilla`: `A` `high` via `cooking`
- `cuajado`: `A` `high` via `cooking`
- `dar un hervor`: `A` `high` via `cooking`
- `desglasar`: `A` `high` via `cooking`
- `desmechado`: `A` `high` via `food`
- `en juliana`: `A` `high` via `cooking`
- `encocado`: `A` `high` via `food`
- `enmolada`: `A` `high` via `cooking`
- `entibiar`: `A` `high` via `cooking`
- `entrecot`: `A` `high` via `cooking`
- `estofar`: `A` `high` via `cooking`

## Limitations

- This audit uses installed local Kaikki/Wiktionary rows and the current food/cooking policy only.
- It does not download sources, mutate packs, write overlays, or change admission behavior.
- Source-capacity totals are not precision-reviewed; use the full-source review packet for sampled precision.
- The common probe list is diagnostic and intentionally small.
