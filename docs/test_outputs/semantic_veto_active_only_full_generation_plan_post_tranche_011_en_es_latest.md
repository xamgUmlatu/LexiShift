...
- Generated: `2026-05-13T20:43:11Z`
- Denominator source-target families: `570`
- Current active-only covered families: `455` (79.8%)
...
| `zipf_below_3_rare` | 52 | 38 | 73.1% | 14 | `affable` -> `gracioso`, `alternation` -> `alternativa`, `battlefront` -> `frontón`, `begrudge` -> `deplorar`, `beret` -> `birrete`, `burndown` -> `quemar` |
...
| `zipf_below_3_rare` | 61 | 41 | 67.2% | 20 | `battlefront` -> `frontón`, `become` -> `acontecer`, `begrudge` -> `deplorar`, `beret` -> `birrete`, `divide` -> `segregar`, `dove` -> `pichón` |
