...
- Status: `ok`
- Generated: `2026-04-25T00:10:35Z`
...
| --- | --- | --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| `Prototype reverse_aux_llm_balanced_plus_source_coverage_filtered_safe, family phrase guard` | `family_all` | `runtime_phrase_guard_only` | 95 | 7 | 7 | 81.6% | 85.3% | 7 | 0 |
| `Prototype reverse_aux_llm_balanced_plus_source_coverage_filtered_safe, active phrase guard` | `active_only` | `runtime_phrase_guard_only` | 95 | 2 | 7 | 81.6% | 90.5% | 21 | 0 |
| `Prototype reverse_aux_llm_balanced_plus_source_coverage_filtered_safe, phrase-control containment guard` | `active_only` | `local_containment_patterns` | 95 | 2 | 7 | 81.6% | 90.5% | 21 | 2 |
| `Prototype reverse_aux_llm_balanced_plus_source_coverage_filtered_safe, surface-POS rescue guard` | `active_only` | `local_containment_patterns_plus_surface_pos` | 95 | 0 | 1 | 97.4% | 98.9% | 21 | 2 |
| `Prototype reverse_aux_llm_balanced_plus_source_coverage_filtered_safe, phrase-control prototype guard` | `active_only` | `semantic_prototype_competition` | 95 | 2 | 9 | 76.3% | 88.4% | 21 | 0 |
...
- Keep the user-facing UX binary, but move the internal experiment from a single evidence string toward prototype admission: context competes against active and shadow example frames, while phrase-control evidence can only abstain through local containment-pattern matches. The best local guard still leaves residual cases on this evaluation slice (`99.0%` accuracy / `97.4%` recall / `0` harmful / `1` false abstains) on `all_dataset_families`; keep broad phrase-control prototype scoring as an overreach control only. The `reverse_aux_llm_balanced_plus_source_coverage_filtered_safe` batch `en-es:example-frame-composite:balanced-plus-source-coverage-filtered-safe-v1-20260425a` is source evidence, but it should clear the required-family contract gate before any promotion or runtime publication claim.
